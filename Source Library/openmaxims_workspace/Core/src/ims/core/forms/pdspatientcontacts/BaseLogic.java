...
public abstract class BaseLogic extends Handlers implements IComponent
{
	...
	{
		return ims.core.domain.PdsPatientContacts.class;
	}
	public final void setContext(ims.framework.UIComponentEngine engine, GenForm form, ims.core.domain.PdsPatientContacts domain)
	{
	...
	{
		this.form.grdCommChannels().colUsageComboBox().clear();
		...
		for(int x = 0; x < lookupCollection.size(); x++)
		{
			this.form.grdCommChannels().colUsageComboBox().newRow(lookupCollection.get(x), lookupCollection.get(x).getText(), lookupCollection.get(x).getImage(), lookupCollection.get(x).getTextColor());
		}
	}
	...
	protected ims.core.domain.PdsPatientContacts domain;
}
