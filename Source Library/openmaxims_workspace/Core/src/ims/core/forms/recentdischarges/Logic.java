...
//		//Clear the selected patient information in the Engine
			...
			if (epis.getPasEventIsNotNull())
			{
				...
				if (epis.getPasEvent().getConsultantIsNotNull())
					if (epis.getPasEvent().getConsultant().getMosIsNotNull())
						if (epis.getPasEvent().getConsultant().getMos().getNameIsNotNull())
						{
							...
							aRow.setCellColConsultantTooltip(epis.getPasEvent().getConsultant().getMos().getName().toString());//WDEV-22737
						}
				if (epis.getPasEvent().getPatientIsNotNull())
				{
					...
					if (	epis.getPasEvent().getPatient().getIdentifiersIsNotNull()
							&& epis.getPasEvent().getPatient().getIdentifiers().size() > 0 
					)
					{
						
						//WDEV-22960
						StringBuilder tooltip = new StringBuilder();
						PatientId patNHSId = epis.getPasEvent().getPatient().getPatId(PatIdType.NHSN);
						PatientId displayId = epis.getPasEvent().getPatient().getDisplayId();
						...
						if(displayId == null)
						{
							if(patNHSId != null)
							{
								...
								aRow.setColHOSNUM(patNHSId.getValue());
								aRow.setCellColHOSNUMTooltip(tooltip.toString());	
							}
						}
						else
						{
							...
							aRow.setColHOSNUM(displayId.getValue().toString());//WDEV-20926
							aRow.setCellColHOSNUMTooltip(tooltip.toString());
						}
						//end WDEV-22960
					...
					if (epis.getPasEvent().getPatient().getSexIsNotNull())
					{
						...
						aRow.setCellColPatSexTooltip(epis.getPasEvent().getPatient().getSex().getText());//WDEV-22737
					}
					
					if (epis.getPasEvent().getPatient().getNameIsNotNull())//WDEV-22209
					{
						if (epis.getPasEvent().getPatient().getName().getSurnameIsNotNull())
						{
							...
							aRow.setCellColSurnameTooltip(epis.getPasEvent().getPatient().getName().getSurname());//WDEV-22737
						}
						if (epis.getPasEvent().getPatient().getName().getForenameIsNotNull())
						{
							...
							aRow.setCellColForenameTooltip(epis.getPasEvent().getPatient().getName().getForename());//WDEV-22737
						}
					}
					
					if (epis.getDischargingWard() != null) //WDEV-22959 
					{
						aRow.setColWard(epis.getDischargingWard().toString());
						aRow.setCellColWardTooltip(epis.getDischargingWard().toString());//WDEV-22737
					}
				...
				if (epis.getDischargeDateTimeIsNotNull())
				{
					...
					aRow.setCellColDateTimeTooltip(epis.getDischargeDateTime().toString());//WDEV-22737
				}
				if (epis.getDischargeDestinationIsNotNull())
				{
					aRow.setColDestination(epis.getDischargeDestination().getText());
					aRow.setCellColDestinationTooltip(epis.getDischargeDestination().getText());//WDEV-22737
				}
			}
		}
	}
	...
	protected void onGrdDischargesSelectionChanged() throws PresentationLogicException 
	{
		...
		if (form.grdDischarges().getValue().getPasEventIsNotNull())
			form.getGlobalContext().Core.setCurrentCareContext(domain.getCurrentCareContext(form.grdDischarges().getValue().getPasEvent()));
		
		PatientShort ps = form.getGlobalContext().Core.getCurrentCareContextIsNotNull() ? form.getGlobalContext().Core.getPatientShort() : (form.grdDischarges().getValue().getPasEventIsNotNull() && form.grdDischarges().getValue().getPasEvent().getPatientIsNotNull() ? form.grdDischarges().getValue().getPasEvent().getPatient() : null);
		form.getGlobalContext().Core.setSelectingPatientForm(ps != null ? engine.getFormName() : null);		
	}
