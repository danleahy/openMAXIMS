...
public class Logic extends BaseLogic
{
	...
	private void loadEWSCombos()
	{
		...
		for (int i = 0; i < voCollConfig.size(); i++)
		{
			...
			if (voConfig.getTypeIsNotNull())
			{
				...
				else if (voConfig.getType().equals(OBSType.CONSCIOUS_LEVEL))//WDEV-20237
				{
					SECSLookupConfigVo voLookupConfig = voConfig.getLookupConfig();
					if (voLookupConfig != null && voLookupConfig.getLookupScores() != null)
					{
				...
				else if (voConfig.getType().equals(OBSType.URINE_OUTPUT))//WDEV-20237
				{
					SECSLookupConfigVo voLookupConfig = voConfig.getLookupConfig();
					if (voLookupConfig != null && voLookupConfig.getLookupScores() != null)
					{
	...
	private void doSave(boolean validateNEWS) throws PresentationLogicException
	{
		...
		if (ConfigFlag.UI.USE_EARLY_WARNING_SYSTEM.getValue() && allRequiredForScoreVitalSignsHaveValues(voVitalSign)) //WDEV-20668
		{
			if (requiresEscalation)
			{
				...
				{
					form.getLocalContext().setEWSMessageId(engine.showMessage(strObsProtocolType+" score is : " + form.getGlobalContext().Core.getVitalSignsEscalationScore(),strObsProtocolType+ " Score", MessageButtons.OK, MessageIcon.INFORMATION));//WDEV-20311  //WDEV-20327
				}
				else
	...
	private boolean requiresEscalation(VitalSignsVo voVitalSign)
	{
		...
		if (voVitalSign.getTemperatureIsNotNull() && voVitalSign.getTemperature().getTemperatureIsNotNull())
		{
			...
			scoreText = getEscalationScoreText(-1, OBSType.TEMPERATURE, voCollConfig, -1, voVitalSign.getTemperature().getTemperature().floatValue()); //WDEV-20237
			if (!scoreText.equals(""))
			{
		...
		{
			String scoreText = null;
			scoreText = getEscalationScoreText(voVitalSign.getBloodPressure().getBPSittingSys().intValue(), OBSType.SYSTOLICBP, voCollConfig, -1, -1);//WDEV-20237
			if (!scoreText.equals(""))
			{
		...
		{
			String scoreText = null;
			scoreText = getEscalationScoreText(voVitalSign.getBloodPressure().getBPLyingSys().intValue(), OBSType.SYSTOLICBP, voCollConfig, -1, -1);//WDEV-20237
			if (!scoreText.equals(""))
			{
				if (!originsOfScore.equals(""))
					originsOfScore.append("\n");
		...
		{
			String scoreText = null;
			scoreText = getEscalationScoreText(voVitalSign.getRespiratory().getRespRate().intValue(), OBSType.RESPIRATORYRATE, voCollConfig, -1, -1);//WDEV-20237
			if (!scoreText.equals(""))
			{
				if (!originsOfScore.equals(""))
					originsOfScore.append("\n");
		...
		{
			String scoreText = null;
			scoreText = getEscalationScoreText(voVitalSign.getPulse().getPulseRateRadial().intValue(), OBSType.PULSE, voCollConfig, -1, -1);//WDEV-20237
			if (!scoreText.equals(""))
			{
				if (!originsOfScore.equals(""))
					originsOfScore.append("\n");
		...
		{
			String scoreText = null;
			scoreText = getEscalationScoreText(voVitalSign.getOxygenSaturation().getOxygenSaturationLevel(), OBSType.OXYGENSATS, voCollConfig, -1, -1);// WDEV-12769  //WDEV-20237
			if (!scoreText.equals(""))
			{
				if (!originsOfScore.equals(""))
					originsOfScore.append("\n");
		...
		{
			String scoreText = null;
			scoreText = getEscalationScoreText(voVitalSign.getPatientCausingConcern().getID(), OBSType.PATIENT_CONCERN, voCollConfig, -1, -1);//WDEV-20237
			if (!scoreText.equals(""))
			{
				if (!originsOfScore.equals(""))
					originsOfScore.append("\n");
		...
		{
			String scoreText = null;
			scoreText = getEscalationScoreText(voVitalSign.getPatientConscious().getID(), OBSType.CONSCIOUS_LEVEL, voCollConfig, -1, -1); //WDEV-20237
			if (!scoreText.equals(""))
			{
				if (!originsOfScore.equals(""))
					originsOfScore.append("\n");
		...
		{
			String scoreText = null;
			scoreText = getEscalationScoreText(voVitalSign.getUrine2mlkgkhr().getID(), OBSType.URINE_OUTPUT, voCollConfig, -1, -1); //WDEV-20237
			if (!scoreText.equals(""))
			{
				if (!originsOfScore.equals(""))
					originsOfScore.append("\n");
	 ...
	 * @return scoreText
	 */
	private String getEscalationScoreText(int value, OBSType type, SECSConfigurationVoCollection voCollConfig, int fractionRate, float tempValue) //WDEV-20237
	{
				...
				if (isRange(type))
				{
					if (voCollConfig.get(i).getRangeConfigIsNotNull() && voCollConfig.get(i).getRangeConfig().getRangeScoresIsNotNull())
					{
						for (int p = 0; p < voCollConfig.get(i).getRangeConfig().getRangeScores().size(); p++)
						{
							...
							if (type.equals(OBSType.TEMPERATURE))//WDEV-20237
							{
...
//									scoreText.append(" < " + voRangeScore.getLessThanIntValue());
	...
	private boolean isRange(OBSType value) //WDEV-20237
	{
		return value != null && (value.equals(OBSType.PULSE) || value.equals(OBSType.RESPIRATORYRATE) || value.equals(OBSType.SYSTOLICBP) || value.equals(OBSType.TEMPERATURE) || value.equals(OBSType.OXYGENSATS) || value.equals(OBSType.FILTEREDO2));// WDEV-12769  //WDEV-20237 //WDEV-20311
	}

	private boolean isScore(OBSType value)//WDEV-20237
	{
		return value != null && (value.equals(OBSType.PATIENT_CONCERN) || value.equals(OBSType.URINE_OUTPUT) || value.equals(OBSType.CONSCIOUS_LEVEL) || value.equals(OBSType.ONSUPPLEMENTALOXYGEN));//WDEV-20237
	}
	...
	private void clearScreen()
	{
		...
		form.decBMI().setValue(null); //WDEV-20327
	}
	...
	private void updateControlState()
	{
		...
		form.btnBMI().setEnabled(form.getMode().equals(FormMode.EDIT)); //WDEV-20327
	}
	...
	{
		if(form.getLocalContext().getEWSMessageIdIsNotNull() && form.getLocalContext().getEWSMessageId().equals(messageBoxId)) //WDEV-20327
		{
