...
import ims.core.forms.vitalsignsbmi.GenForm.grpShowByEnumeration;
...
public class Logic extends BaseLogic
{
	...
	protected void onFormOpen() throws FormOpenException
	{
		...
		{
			//WDEV-19389 - start
			if (isVitalSignsBMI() && form.getGlobalContext().Core.getVitalSign() == null && form.getGlobalContext().Core.getVitalSignsBMISearchCriteriaIsNotNull())
			{
				if (form.getGlobalContext().Core.getVitalSign() == null && !(form.getGlobalContext().Core.getCurrentCareContextIsNotNull() && form.getGlobalContext().Core.getCurrentCareContext().equals(form.getGlobalContext().Core.getVitalSignsBMISearchCriteria().getCareContext())))
					form.getGlobalContext().Core.setVitalSignsBMISearchCriteria(null);
				
				doSeachCriteria(form.getGlobalContext().Core.getVitalSignsBMISearchCriteria());
			}
			else if (isVitalSignsBMIAlias() && form.getGlobalContext().Core.getVitalSign() == null && form.getGlobalContext().Core.getVitalSignsBMIAliasSearchCriteriaIsNotNull())
			{
				if (form.getGlobalContext().Core.getVitalSign() == null && !(form.getGlobalContext().Core.getCurrentCareContextIsNotNull() && form.getGlobalContext().Core.getCurrentCareContext().equals(form.getGlobalContext().Core.getVitalSignsBMIAliasSearchCriteria().getCareContext())))
					form.getGlobalContext().Core.setVitalSignsBMIAliasSearchCriteria(null);
				
				doSeachCriteria(form.getGlobalContext().Core.getVitalSignsBMIAliasSearchCriteria());
			}
			else
			{
				try {
	...
	private void doSeachCriteria(VitalSignsFormsSearchCriteriaVo vitalSignsBMIAliasSearchCriteria)
	{
		if (vitalSignsBMIAliasSearchCriteria != null)
		{
			try
			{
				setSearchCriteria(vitalSignsBMIAliasSearchCriteria);
				onBtnViewClick();
			}
			catch (PresentationLogicException e) {
				throw new DomainRuntimeException();
	...
	private boolean isVitalSignsBMI()
	{
		return engine.getFormName().equals(form.getForms().Core.VitalSignsBMI);
	}

	private boolean isVitalSignsBMIAlias()
	{
		return engine.getFormName().equals(form.getForms().Core.VitalSignsBMIAlias);
	}
	...
	protected void onBtnViewClick() throws ims.framework.exceptions.PresentationLogicException
	{
		...
		if (form.getGlobalContext().Core.getVitalSign() == null )
		{
			if (isVitalSignsBMI())
			{
				form.getGlobalContext().Core.setVitalSignsBMISearchCriteria(getSearchCriteria());
			}
			else if (isVitalSignsBMIAlias())
			{
				form.getGlobalContext().Core.setVitalSignsBMIAliasSearchCriteria(getSearchCriteria());
			}				
		}
