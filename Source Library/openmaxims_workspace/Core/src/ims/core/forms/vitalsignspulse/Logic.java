...
import ims.core.forms.vitalsignspulse.GenForm.grpShowByEnumeration;
...
public class Logic extends BaseLogic
{
	...
	protected void onFormOpen() throws ims.framework.exceptions.FormOpenException
	{
		...
		{
			//WDEV-19389 - start
			if (isVitalSignsPulse() && form.getGlobalContext().Core.getVitalSign() == null && form.getGlobalContext().Core.getVitalSignsPulseSearchCriteriaIsNotNull())
			{
				if (form.getGlobalContext().Core.getVitalSign() == null && !(form.getGlobalContext().Core.getCurrentCareContextIsNotNull() && form.getGlobalContext().Core.getCurrentCareContext().equals(form.getGlobalContext().Core.getVitalSignsPulseSearchCriteria().getCareContext())))
					form.getGlobalContext().Core.setVitalSignsPulseSearchCriteria(null);
				
				doSeachCriteria(form.getGlobalContext().Core.getVitalSignsPulseSearchCriteria());
			}
			else if (isVitalSignsPulseAlias() && form.getGlobalContext().Core.getVitalSign() == null && form.getGlobalContext().Core.getVitalSignsPulseAliasSearchCriteriaIsNotNull())
			{
				if (form.getGlobalContext().Core.getVitalSign() == null && !(form.getGlobalContext().Core.getCurrentCareContextIsNotNull() && form.getGlobalContext().Core.getCurrentCareContext().equals(form.getGlobalContext().Core.getVitalSignsPulseAliasSearchCriteria().getCareContext())))
					form.getGlobalContext().Core.setVitalSignsPulseAliasSearchCriteria(null);
				
				doSeachCriteria(form.getGlobalContext().Core.getVitalSignsPulseAliasSearchCriteria());
			}
			else
			{
				try {
	...
	private boolean isVitalSignsPulse()
	{
		return engine.getFormName().equals(form.getForms().Core.VitalSignsPulse);
	}

	private boolean isVitalSignsPulseAlias()
	{
		return engine.getFormName().equals(form.getForms().Core.VitalSignsPulseAlias);
	}
	...
	private void setViewType(Integer searchType)
	{
		...
		{
		case LIST_BY_DAY:
			form.grpShowBy().setValue(grpShowByEnumeration.rdoPulseByDay);
			break;
		case LIST_BY_DATES:
			form.grpShowBy().setValue(grpShowByEnumeration.rdoPulseByDates);
			break;
		}				
	}
	...
	private Integer getViewType()
	{
		...
		if (grpShowByEnumeration.rdoPulseByDay.equals(searchType))
		{
			return LIST_BY_DAY;
		}
		if (grpShowByEnumeration.rdoPulseByDates.equals(searchType))
		{
	...
	protected void onBtnViewClick() throws PresentationLogicException
	{
		...
		{
			if (isVitalSignsPulse())
			{
				form.getGlobalContext().Core.setVitalSignsPulseSearchCriteria(getSearchCriteria());
			}
			else if (isVitalSignsPulseAlias())
			{
				form.getGlobalContext().Core.setVitalSignsPulseAliasSearchCriteria(getSearchCriteria());
			}				
		}
		clearFields();
		listPulse();
	...
	private void listPulse()
	{
		...
		GraphingPulse point;
		if (voCollVitalSigns != null && voCollVitalSigns.size() > 0) // WDEV-16175 
		{
