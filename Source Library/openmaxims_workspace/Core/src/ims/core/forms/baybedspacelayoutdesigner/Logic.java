...
public class Logic extends BaseLogic
{
	...
	private void initialize()
	{
		...
		loadFloorLayouts();
		form.bedPlanner().addBedImage(Bed.WHITE_BED.getType());
		form.bedPlanner().addBedImage(Bed.WHITE_CHAIR.getType());
	}
		...
		if (value != null)
		{
			...
			if (value.getBedSpaces() != null)
			{
				for (int x = 0; x < value.getBedSpaces().size(); x++)
				{
					...
					try
					{
						bed = new Bed(voBed.getID_BedSpace(), isChair(voBed) ? Bed.WHITE_CHAIR.getType() : Bed.WHITE_BED.getType(), voBed.getVml(), Position.parse(voBed.getTextPosition()), Position.parse(voBed.getImagePosition()));
					}
	...
	{
		boolean savedRecord = form.getLocalContext().getCurrentRecord() != null && form.getLocalContext().getCurrentRecord().getID_FloorBedSpaceLayout() != null;
		boolean newRecord = form.getLocalContext().getCurrentRecord() == null || form.getLocalContext().getCurrentRecord().getID_FloorBedSpaceLayout() == null;
		
		form.chkIsTemplate().setEnabled(FormMode.EDIT.equals(form.getMode()) && (savedRecord &&  PreActiveActiveInactiveStatus.PREACTIVE.equals(form.cmbStatus().getValue())) || newRecord);
	}
	...
	{
		form.getGlobalContext().Core.getBedSpaceProperty();
	}
	//WDEV-19653
	@Override
	protected void onBedPlannerBedRemoved(Bed bed)
	{
		if (bed.getId() != null && bed.getId().intValue() < 0 && form.getGlobalContext().Core.getBedSpaceProperty().getId().equals(bed.getId())) //unsaved beds receive a dummy id (negative) 
		{
			form.getGlobalContext().Core.setBedSpaceProperty(null);
		}
