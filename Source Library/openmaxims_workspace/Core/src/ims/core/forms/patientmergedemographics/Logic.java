...
//import ims.core.patient.domain.objects.NonUniquePatientId;
...
import ims.core.vo.NonUniquePatientIdVo;
import ims.core.vo.NonUniquePatientIdVoCollection;
...
public class Logic extends BaseLogic
{
	...
	private boolean switchDUPIdentifiers() 
	{
		...
		for(int i = 0; i<resultIdentifiers.size(); i++)
		{
			...
			for(int j=0; j<sourceIdentifiers.size(); j++)
			{
				...
				if(sourceId.getType().equals(resultId.getType()) && sourceId.getIdValue().equals(resultId.getIdValue()))
				{
	...
	private void populateCell(DynamicGridCell cell, PatientMergeFields rowValue, PatientMergeVo patient, Patient result, boolean isFromSource, boolean isApplyAllClicked) 
	{
		...
		else if(PatientMergeFields.MEDICAL_CARD_INFORMATION.equals(rowValue))
		{
			...
			if(result != null)
			{
				result.setNationalHealthCover(patient.getNationalHealthCover());
			}
		}
		...
		for(int j = identifiers.size() - 1; j>=0; j--)
		{
			...
			for(int i=0; i<parentRow.getRows().size(); i++)
			{
				if(parentRow.getRows().get(i).getValue().equals(identifiers.get(j).getType()))
				{
					...
					{
						String destinationIdentifierValue = identifiers.get(j).getValue();
	...
	private boolean save() 
	{
		...
		MergeRequestVo voMergeRequest = populateMergeRequest();
...
//			form.getLocalContext().setResultPatient(domain.savePatientAndmergeRequest(form.getLocalContext().getResultPatient(), patientBeforeMerge, voMergeRequest, foundDUP ? form.getLocalContext().getSourcePatientFull() : null, form.getLocalContext().getSourcePatientIdentifierCollection()));
	...
	//http://jira/browse/WDEV-20242
	private NonUniquePatientIdVoCollection getNonUniqueIds(PatientIdCollection patientIdCollection)
	{
		if(patientIdCollection == null)
			return null;
		
		NonUniquePatientIdVoCollection nonUniqIdVoColl = new NonUniquePatientIdVoCollection();
		
		
		for (int i = 0; i < patientIdCollection.size(); i++)
		{
			NonUniquePatientIdVo survivingPatId = new NonUniquePatientIdVo();
			PatientId nonSurvivingPatId = patientIdCollection.get(i);

			if(nonSurvivingPatId != null)
			{
				survivingPatId.setID_NonUniquePatientId(nonSurvivingPatId.getID_PatientId());
				survivingPatId.setType(nonSurvivingPatId.getType());
				survivingPatId.setValue(nonSurvivingPatId.getValue());
				survivingPatId.setVerified(nonSurvivingPatId.getVerified());
				survivingPatId.setMerged(nonSurvivingPatId.getMerged());
				survivingPatId.setDuplicateNHSNum(nonSurvivingPatId.getDuplicateNHSNum());

				nonUniqIdVoColl.add(survivingPatId);
			}
			
		}
		
		return nonUniqIdVoColl;
...
//		patientBeforeMerge.setIdentifiers(form.getLocalContext().getDestinationPatient().getIdentifiers());
		patientBeforeMerge.setIdentifiers(getNonUniqueIds(form.getLocalContext().getDestinationPatient().getIdentifiers())); //WDEV-20421
		...
		patientBeforeMerge.setInsurance(form.getLocalContext().getDestinationPatient().getNationalHealthCover());
