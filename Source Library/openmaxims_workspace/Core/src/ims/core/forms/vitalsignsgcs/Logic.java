...
import ims.core.forms.vitalsignsgcs.GenForm.grpShowByEnumeration;
...
public class Logic extends BaseLogic 
{
	...
	protected void onFormOpen() throws FormOpenException
	{
		...
		{
			//WDEV-19389 - start
			if (isVitalSignsGCS() && form.getGlobalContext().Core.getVitalSign() == null && form.getGlobalContext().Core.getVitalSignsGCSSearchCriteriaIsNotNull())
			{
				if (form.getGlobalContext().Core.getVitalSign() == null && !(form.getGlobalContext().Core.getCurrentCareContextIsNotNull() && form.getGlobalContext().Core.getCurrentCareContext().equals(form.getGlobalContext().Core.getVitalSignsGCSSearchCriteria().getCareContext())))
					form.getGlobalContext().Core.setVitalSignsGCSSearchCriteria(null);
				
				doSeachCriteria(form.getGlobalContext().Core.getVitalSignsGCSSearchCriteria());
			}
			else if (isVitalSignsGCSAlias() && form.getGlobalContext().Core.getVitalSign() == null && form.getGlobalContext().Core.getVitalSignsGCSAliasSearchCriteriaIsNotNull())
			{
				if (form.getGlobalContext().Core.getVitalSign() == null && !(form.getGlobalContext().Core.getCurrentCareContextIsNotNull() && form.getGlobalContext().Core.getCurrentCareContext().equals(form.getGlobalContext().Core.getVitalSignsGCSAliasSearchCriteria().getCareContext())))
					form.getGlobalContext().Core.setVitalSignsGCSAliasSearchCriteria(null);
				
				doSeachCriteria(form.getGlobalContext().Core.getVitalSignsGCSAliasSearchCriteria());
			}
			else
			{
				try {
	...
	private boolean isVitalSignsGCS()
	{
		return engine.getFormName().equals(form.getForms().Core.VitalSignsGCS);
	}

	private boolean isVitalSignsGCSAlias()
	{
		return engine.getFormName().equals(form.getForms().Core.VitalSignsGCSAlias);
	}
	...
	private VitalSignsFormsSearchCriteriaVo getSearchCriteria()
	{
		...
		searchCriteria.setByDay(form.dteShowByDate().getValue());
	...
	private void setSearchCriteria(VitalSignsFormsSearchCriteriaVo vitalSignsFormsSearchCriteriaVo) throws PresentationLogicException
	{
		...
		onRadioButtongrpShowByValueChanged();
		form.dteShowByDate().setValue(vitalSignsFormsSearchCriteriaVo.getByDay());
	...
	//WDEV-20191
	protected String[] validateValuesforGlasgowComaScale()
	{
		ArrayList<String> error = new ArrayList<String>();
		if (form.cmbEye().getValue() == null ||form.cmbMotor().getValue() == null || form.cmbVerbal().getValue() == null )
		{
			error.add("Please enter all values for Glasgow Coma Scale!");
	...
	protected void onBtnViewClick() throws PresentationLogicException
	{
		...
		if (form.getGlobalContext().Core.getVitalSign() == null )
		{
			if (isVitalSignsGCS())
			{
				form.getGlobalContext().Core.setVitalSignsGCSSearchCriteria(getSearchCriteria());
			}
			else if (isVitalSignsGCSAlias())
			{
				form.getGlobalContext().Core.setVitalSignsGCSAliasSearchCriteria(getSearchCriteria());
			}				
		}
