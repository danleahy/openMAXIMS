...
import ims.core.forms.patientcorrespondencetemplates.GenForm.Group1Enumeration;
...
public class Logic extends BaseLogic
{
	...
	private void initialize() throws PresentationLogicException 
	{
		...
		DocumentCategoryToFormCfgVo documentCategoryConfig = domain.getDocumentCategoryConfig(form.getForms().Core.PatientCorrespondenceTemplates.getID());
		...
		{
			form.lbl13().setVisible(true);
			form.Group1().setVisible(Group1Enumeration.rdoYes, true);
			form.Group1().setVisible(Group1Enumeration.rdoNo, true);
		}
		else
		{
			form.lbl13().setVisible(false);
			form.Group1().setVisible(Group1Enumeration.rdoYes, false);
			form.Group1().setVisible(Group1Enumeration.rdoNo, false);
		}
		//---------
	}
	...
	private String[] validateUIRules() 
	{
		...
		{
			uiErrors.add("Responsible HCP is mandatory.");
		}
	...
	{
		// WDEV-18219
		if (ConfigFlag.UI.PATIENT_CORRESPONDENCE_TEMPLATES_DEFAULT_SPECIALTY_FROM_CARE_CONTEXT.getValue())
		{
			if (form.cmbCContext().getValue() != null && form.cmbCContext().getValue() instanceof CareContextForPatientDocumentVo && form.cmbCContext().getValue().getPasEventIsNotNull())
				form.getLocalContext().setSelectedSpecialty(form.cmbCContext().getValue().getPasEvent().getSpecialty());
			else if (form.cmbEpisode().getValue() != null && form.cmbEpisode().getValue() instanceof EpisodeofCareShortVo)
				form.getLocalContext().setSelectedSpecialty(form.cmbEpisode().getValue().getSpecialty());
			
			bindAndSelectSpecialty();
		}
		...
		else if (form.cmbCContext().getValue() == null)
		{
			selectEpisodeOfCare();
		}
		else
	...
	private void bindAndSelectSpecialty() 
	{
		...
		Specialty specialtyFromCC = null;// WDEV-18219 
		...
		if(Boolean.TRUE.equals(ConfigFlag.UI.PATIENT_CORRESPONDENCE_TEMPLATES_DEFAULT_SPECIALTY_FROM_CARE_CONTEXT.getValue()) && form.cmbCContext().getValue()!= null &&  form.cmbCContext().getValue() instanceof CareContextForPatientDocumentVo && form.cmbCContext().getValue().getPasEventIsNotNull())
		{
			specialtyFromCC = form.cmbCContext().getValue().getPasEvent().getSpecialty();//WDEV-18219 
		}
		...
		if(form.dyngrdReports().getValue() instanceof DocumentCategoryConfigShortVo && ((DocumentCategoryConfigShortVo) form.dyngrdReports().getValue()).getSpecialty() != null)//WDEV-13992
		{
			...
			if(specialtyFromCC != null && specialtyFromCC != null && !epSpecialty.contains(specialtyFromCC))//WDEV-18219 
			{
				epSpecialty.add(specialtyFromCC);//WDEV-18219 
			}
		}
	...
	private String getTextForCareContext(CareContextForPatientDocumentVo cc) 
	{
		...
		//WDEV-18219 
		if (Boolean.TRUE.equals(ConfigFlag.UI.PATIENT_CORRESPONDENCE_TEMPLATES_DEFAULT_SPECIALTY_FROM_CARE_CONTEXT.getValue()) && cc.getPasEventIsNotNull() && cc.getPasEvent().getSpecialtyIsNotNull())
		{
			result.append(" - " +  cc.getPasEvent().getSpecialty().getText());
		}
	...
	private void selectClinicalContact() 
	{
		...
		else
		{
			...
			{
				form.getLocalContext().setSelectedSpecialty(form.cmbCContext().getValue().getPasEvent().getSpecialty());
				form.cmbSpecialty().setValue(form.cmbCContext().getValue().getPasEvent().getSpecialty());
			} 
			else if( form.cmbEpisode().getValue() != null && form.cmbEpisode().getValue() instanceof EpisodeofCareShortVo)
			{
				form.getLocalContext().setSelectedSpecialty(form.cmbEpisode().getValue().getSpecialty());
