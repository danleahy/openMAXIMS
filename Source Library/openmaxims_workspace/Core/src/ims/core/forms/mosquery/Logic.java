...
public class Logic extends BaseLogic
{
	...
	protected void onQmbMOSTextSubmited(String value) throws ims.framework.exceptions.PresentationLogicException
	{
		...
		//WDEV-15687
		IMos[] mosColl =null;
		if (form.getLocalContext().getMedicGradeIsNotNull() || Boolean.TRUE.equals(form.getLocalContext().getIsMedic())) //WDEV-21788
		{
	...
	public void initializeLocalConsultant(MosType mosType,Boolean isALocalConsultant) 
	{
		clearSpecificSearchContexts();
		form.getLocalContext().setMOSSearchType(mosType);
		form.getLocalContext().setIsALocalConsultant(isALocalConsultant);
	}
	...
	{
		return form.qmbMOS().isEnabled();
	}

	//WDEV-20429
	public Boolean isVisible()
	{
		return form.qmbMOS().getVisible();
	}

	public void initializeResponsibleHcp(MosType mosType, Boolean isResponsibleHCP, ServiceRefVo service, ServiceFunction function)
	{	
		clearSpecificSearchContexts();
		form.getLocalContext().setMOSSearchType(mosType);
		form.getLocalContext().setIsResponsibleHcp(isResponsibleHCP);
		form.getLocalContext().setService(service);
		form.getLocalContext().setFunction(function);
	}
	private void clearSpecificSearchContexts()
	{		
		form.getLocalContext().setMosDiscipline(null);
		form.getLocalContext().setMOSLocation(null);		
		form.getLocalContext().setCanReferPatient(null);
		form.getLocalContext().setIsResponsibleHcp(null);
		form.getLocalContext().setMedicGrade(null);
		form.getLocalContext().setService(null);
		form.getLocalContext().setFunction(null);
		form.getLocalContext().setIsALocalConsultant(null);
		form.getLocalContext().setIsMedic(null); //WDEV-21788
	}

	public void initialize(MosType mosType, HcpDisType mosDiscipline, Boolean isResponsibleHcp)
	{
		clearSpecificSearchContexts();
		form.getLocalContext().setMOSSearchType(mosType);
		form.getLocalContext().setMosDiscipline(mosDiscipline);
