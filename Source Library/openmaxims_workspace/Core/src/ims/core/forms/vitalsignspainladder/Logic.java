...
import ims.core.forms.vitalsignspainladder.GenForm.grpShowByEnumeration;
import ims.core.vo.AuthoringInformationVo;
import ims.core.vo.CareContextShortVo;
import ims.core.vo.VSPainLadderVo;
import ims.core.vo.VSPainLadderVoCollection;
...
public class Logic extends BaseLogic
{
	...
	private void initialize()
	{
		...
		if(vitalSignGlobalContext != null  && vitalSignGlobalContext.getPain() != null)
		{
			...
			listPain();
			//WDEV-20192
			displayRecord(form.getLocalContext().getPains().get(form.getLocalContext().getPains().size()-1));
		...
		{
    		//WDEV-19389 - start
    		if (isVitalSignsPainLadder() && form.getGlobalContext().Core.getVitalSign() == null && form.getGlobalContext().Core.getVitalSignsPLSearchCriteriaIsNotNull())
    		{
    			if (form.getGlobalContext().Core.getVitalSign() == null && !(form.getGlobalContext().Core.getCurrentCareContextIsNotNull() && form.getGlobalContext().Core.getCurrentCareContext().equals(form.getGlobalContext().Core.getVitalSignsPLSearchCriteria().getCareContext())))
    				form.getGlobalContext().Core.setVitalSignsPLSearchCriteria(null);
    			
    			doSeachCriteria(form.getGlobalContext().Core.getVitalSignsPLSearchCriteria());
    		}
    		else if (isVitalSignsPainLadderAlias() && form.getGlobalContext().Core.getVitalSign() == null && form.getGlobalContext().Core.getVitalSignsPLAliasSearchCriteriaIsNotNull())
    		{
    			if (form.getGlobalContext().Core.getVitalSign() == null && !(form.getGlobalContext().Core.getCurrentCareContextIsNotNull() && form.getGlobalContext().Core.getCurrentCareContext().equals(form.getGlobalContext().Core.getVitalSignsPLAliasSearchCriteria().getCareContext())))
    				form.getGlobalContext().Core.setVitalSignsPLAliasSearchCriteria(null);
    			
    			doSeachCriteria(form.getGlobalContext().Core.getVitalSignsPLAliasSearchCriteria());
    		}
    		else
    		{
    			try {
	...
	//WDEV-20192
	private VitalSignsVoCollection extractPains(VitalSignsVoCollection voPain) 
	{
	
		VitalSignsVoCollection pains = new VitalSignsVoCollection(); 
		
		for(VitalSignsVo pain : voPain)
		{
			if( pain.getPainIsNotNull() )
				pains.add(pain);
		}
		return pains;
	}
	
	private boolean isVitalSignsPainLadder()
	{
		return engine.getFormName().equals(form.getForms().Core.VitalSignsPainLadder);
	}

	private boolean isVitalSignsPainLadderAlias()
	{
		return engine.getFormName().equals(form.getForms().Core.VitalSignsPainLadderAlias);
	}
	...
	private VitalSignsFormsSearchCriteriaVo getSearchCriteria()
	{
		...
		searchCriteria.setByDay(form.dteDay().getValue());
	...
	private void setSearchCriteria(VitalSignsFormsSearchCriteriaVo vitalSignsFormsSearchCriteriaVo) throws PresentationLogicException
	{
		...
		onRadioButtongrpShowByValueChanged();
		form.dteDay().setValue(vitalSignsFormsSearchCriteriaVo.getByDay());
...
//							nIndex = x;
			...
			//WDEV-20192
			VitalSignsVoCollection extractPains = extractPains(voCollVitals);
			form.getLocalContext().setPains(extractPains);
			form.getLocalContext().setCurrentIndex(new Integer(extractPains.size()-1));
			displayCurrentPain();
		}
		else
		{
	...
	protected void onBtnViewClick() throws ims.framework.exceptions.PresentationLogicException
	{
		...
		{
			if (isVitalSignsPainLadder())
			{
				form.getGlobalContext().Core.setVitalSignsPLSearchCriteria(getSearchCriteria());
			}
			else if (isVitalSignsPainLadderAlias())
			{
				form.getGlobalContext().Core.setVitalSignsPLAliasSearchCriteria(getSearchCriteria());
			}				
		}
