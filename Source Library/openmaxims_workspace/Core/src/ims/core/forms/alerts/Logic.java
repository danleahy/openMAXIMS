...
import ims.core.forms.alerts.GenForm.groupFilterEnumeration;
import ims.core.helper.ResetPIDBarHelper;
import ims.core.vo.AlertsResultsSearchCriteriaVo;
...
public class Logic extends BaseLogic
{
	...
	{
		form.getLocalContext().setAlertsThatCanBeViewedOrEdited(getAlertsCategoriesThanCanBeViewedOrEdit(false));
		form.getLocalContext().setAlertsThatCanBeEdited(getAlertsCategoriesThanCanBeViewedOrEdit(true));
		
		initialise();	
		initializeAlertsForTriageForm(args);//	WDEV-14413
		
		//WDEV-19389 
		if (!(form.getGlobalContext().Core.getPatientShortIsNotNull() && form.getGlobalContext().Core.getAlertsResultsSearchCriteriaIsNotNull() && form.getGlobalContext().Core.getAlertsResultsSearchCriteria().getPatientIsNotNull() && form.getGlobalContext().Core.getPatientShort().equals(form.getGlobalContext().Core.getAlertsResultsSearchCriteria().getPatient())))
			form.getGlobalContext().Core.setAlertsResultsSearchCriteria(null);
		
		if(form.getGlobalContext().Core.getAlertsResultsSearchCriteriaIsNotNull())
		{
			setSearchCriteria(form.getGlobalContext().Core.getAlertsResultsSearchCriteria());
			try
			{
	...
	private void initialise() 
	{
		...
		//loadCategories(false); //Hierarchichal functionality - Select Alert Category then Alert Type
	...
	protected void onRadioButtongroupFilterValueChanged() throws ims.framework.exceptions.PresentationLogicException
	{
		...
		form.getGlobalContext().Core.setAlertsResultsSearchCriteria(getSearchCriteria());
	}
	
	private AlertsResultsSearchCriteriaVo getSearchCriteria()
	{
		AlertsResultsSearchCriteriaVo searchCriteria = new AlertsResultsSearchCriteriaVo();
	...
	private void setSearchCriteria(AlertsResultsSearchCriteriaVo alertsResultsSearchCriteria)
	{
		setViewType(alertsResultsSearchCriteria.getViewType());		
	}
	...
	private boolean savePatientCoreClinicalData(boolean bNoKnownAlerts) 
	{
		...
		String[] errors = voPatientNoAlertInfo.validate(validateUIRules()); // WDEV-18756 
	...
	private void populateGrid()
	{
		...
		PatientAlertCollection voCollection = domain.listPatientAlerts(form.getGlobalContext().Core.getPatientShort(), Boolean.FALSE, engine.getLoggedInRole());
		...
		form.getContextMenus().hideAllGenericGridMenuItems();
		form.getContextMenus().getGenericGridAddItem().setVisible(form.getLocalContext().getAlertsThatCanBeEdited() != null && form.getLocalContext().getAlertsThatCanBeEdited().size() > 0);
	...
	{
		//WDEV-22977
		boolean alertSaved = form.getLocalContext().getAlertRecord() != null && form.getLocalContext().getAlertRecord().getID_PatientAlert() != null;

		if (alertSaved)
		{
			form.ctnDetails().cmbCategory().setEnabled(!alertSaved);
			form.ctnDetails().cmbAlerts().setEnabled(!alertSaved);
			form.ctnDetails().cmbAlerts().setEnabled(!alertSaved);
			form.ctnDetails().txtComment().setEnabled(true);

			form.ctnDetails().cmbSource().setEnabled(!alertSaved);
			form.ctnDetails().pdtIdentified().setEnabled(!alertSaved);
			form.ctnDetails().setcustomControlAuthoringInfoEnabled(!alertSaved);
			form.ctnDetails().setCollapsed(false);
		}
		else if (!alertSaved)
		{
			...
			form.ctnDetails().customControlAuthoringInfo().setIsRequiredPropertyToControls(Boolean.TRUE); // WDEV-18756 
			form.ctnDetails().setCollapsed(false);
		}
		//WDEV-22977 ends here
	}
	...
	private void updateControlsState() 
	{
		...
		{
			form.btnNew().setEnabled(form.getLocalContext().getAlertsThatCanBeEdited() != null && form.getLocalContext().getAlertsThatCanBeEdited().size() > 0);
		}
	...
	protected void onRIEDialogClosed(DialogResult result) throws PresentationLogicException
	{
		...
		try
		{
			domain.savePatientAlertCategories(voPatientAlert.getPatient());
		}
