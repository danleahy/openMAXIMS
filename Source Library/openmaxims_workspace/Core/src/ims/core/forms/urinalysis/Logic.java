...
import ims.core.forms.urinalysis.GenForm.grpFilterByEnumeration;
...
//		form.grpFilterBy().setValue(GenForm.grpFilterByEnumeration.rdoActive);
		...
		//WDEV-19389 
		if (!(form.getGlobalContext().Core.getCurrentCareContextIsNotNull() && form.getGlobalContext().Core.getUrinalysisSearchCriteriaIsNotNull() && form.getGlobalContext().Core.getCurrentCareContext().equals(form.getGlobalContext().Core.getUrinalysisSearchCriteria().getCareContext())))
			form.getGlobalContext().Core.setUrinalysisSearchCriteria(null);
		
		if(form.getGlobalContext().Core.getUrinalysisSearchCriteriaIsNotNull())
		{
			setSearchCriteria(form.getGlobalContext().Core.getUrinalysisSearchCriteria());
			try
			{
	...
	private void setViewType(Boolean isActive)
	{
		if(Boolean.TRUE.equals(isActive))
			form.grpFilterBy().setValue(grpFilterByEnumeration.rdoActive);
		else
			form.grpFilterBy().setValue(grpFilterByEnumeration.rdoInactive);			
	}
