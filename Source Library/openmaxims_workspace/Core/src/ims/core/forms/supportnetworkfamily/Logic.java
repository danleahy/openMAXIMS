...
import ims.core.forms.supportnetworkfamily.GenForm.groupFilterEnumeration;
...
public class Logic extends BaseLogic
{
	...
	protected void onFormOpen() throws PresentationLogicException
	{
		...
		//WDEV-19389 
		if(engine.getFormName().equals(form.getForms().Core.SupportNetworkFamily))
		{
			if (!(form.getGlobalContext().Core.getPatientShortIsNotNull() && form.getGlobalContext().Core.getSupportNetworkFamilySearchCriteriaIsNotNull() && form.getGlobalContext().Core.getPatientShort().equals(form.getGlobalContext().Core.getSupportNetworkFamilySearchCriteria().getPatient())))
				form.getGlobalContext().Core.setSupportNetworkFamilySearchCriteria(null);
		
			doSearchCriteria(form.getGlobalContext().Core.getSupportNetworkFamilySearchCriteria());			
		}
		if(engine.getFormName().equals(form.getForms().Core.SupportNetworkFamilyNursingAssessment))
		{
			if (!(form.getGlobalContext().Core.getPatientShortIsNotNull() && form.getGlobalContext().Core.getSupportNetworkFamilyAliasSearchCriteriaIsNotNull() && form.getGlobalContext().Core.getPatientShort().equals(form.getGlobalContext().Core.getSupportNetworkFamilyAliasSearchCriteria().getPatient())))
				form.getGlobalContext().Core.setSupportNetworkFamilyAliasSearchCriteria(null);
		
			doSearchCriteria(form.getGlobalContext().Core.getSupportNetworkFamilyAliasSearchCriteria());
		}
		//WDEV-19389 - end
	
	}
	
	private void doSearchCriteria(SupportNetworkSearchCriteriaVo supportNetworkFamilySearchCriteria) throws PresentationLogicException
	{
		if(supportNetworkFamilySearchCriteria != null)
		{
			setSearchCriteria(supportNetworkFamilySearchCriteria);
	...
	private SupportNetworkSearchCriteriaVo getSearchCriteria()
	{
	...
	private void setSearchCriteria(SupportNetworkSearchCriteriaVo supportNetworkSearchCriteriaVo)
	{
		setViewType(supportNetworkSearchCriteriaVo.getViewType());		
	}
	...
	protected void onRadioButtongroupFilterValueChanged() throws ims.framework.exceptions.PresentationLogicException
	{
		...
		if (engine.getFormName().equals(form.getForms().Core.SupportNetworkFamily))
			form.getGlobalContext().Core.setSupportNetworkFamilySearchCriteria(getSearchCriteria()); //WDEV-19389
		else if (engine.getFormName().equals(form.getForms().Core.SupportNetworkFamilyNursingAssessment))
			form.getGlobalContext().Core.setSupportNetworkFamilyAliasSearchCriteria(getSearchCriteria());
		form.setMode(FormMode.VIEW);
	}
