...
package ims.core.forms.eprescribing;
...
import ims.framework.cn.events.CustomUrlWindowChangedEvent;
...
public class Logic extends BaseLogic
{
	...
	protected void onFormOpen(Object[] args) throws ims.framework.exceptions.PresentationLogicException
	{
		...
		if (ConfigFlag.UI.EPRES_MESSAGE_URL.getValue() != "")
			form.htmBrowser().setIFrameValue(ConfigFlag.UI.EPRES_MESSAGE_URL.getValue());

		if (ConfigFlag.UI.EPRES_LAUNCH_IE.getValue())
		{
			//Setting FullScreen (even to False) resets the values of the AddressBar and ToolBar properties to True. 
			//Disable the address bar and toolbars after you set the FullScreen property.
			try
			{
				List<WindowParam> params = new ArrayList<WindowParam>();
								  ...
								  params.add(new WindowParam("Resizable","false"));		
								  params.add(new WindowParam("Visible","true"));		
								  //params.add(new WindowParam("Height","1024"));
								  //params.add(new WindowParam("Width","1280"));
				
				engine.openCustomUrl(geturl(), params, true);
				
				//Set Edit Mode for Form to prevent changing patient context
		...
		{
			engine.open(form.getForms().Core.EpresDialog,true,true);
		}
	}

	private String geturl() throws ConfigurationException
	{		 		
		try {
			...
			if (engine.getLoggedInUser() != null)
			{
				...
				if (!ConfigFlag.GEN.RELEASE_MODE.getValue())
				{
					try
					{
						System.out.println("Ascribe Data:" + xmlData);
					} catch (Exception e)
					{
						e.printStackTrace();
						System.out.println(e.getMessage());
				...
				else
				{
					...
					if (!strToken.contains("<BrokenRule"))
					{
						if (strToken.indexOf("Token Id") > 0)
						{
							...
							return ConfigFlag.UI.EPRES_URL.getValue() + "?token=" + strToken3;
						}
					}				
				}
...
//		    root.addElement("HOSPNUM", form.getGlobalContext().Core.getPatientShort().getHospnum().getIdValue());					
	...
	protected void onFormDialogClosed(FormName formName, DialogResult result) throws PresentationLogicException {
		...
		//this is a work around to launch an instance of IE through COM to free up the patient select in eprescrbing			
		JISession session = null;
		JIComServer comServer = null;
		IJIDispatch ieObjectDispatch = null;
		try {
			
			session = JISession.createSession(EnvironmentConfig.getEpresDomain(),EnvironmentConfig.getEpresDomainUser(),EnvironmentConfig.getEpresDomainPassword());
			comServer = new JIComServer(JIProgId.valueOf("InternetExplorer.Application"),"localhost",session);
			IJIComObject ieObject = comServer.createInstance();
			...
			ieObjectDispatch.callMethod("Navigate2",new Object[]{new JIString(ConfigFlag.UI.EPRES_URL.getValue() + "?token=" + form.getGlobalContext().Core.getsecurityToken()),JIVariant.OPTIONAL_PARAM(),JIVariant.OPTIONAL_PARAM(),JIVariant.OPTIONAL_PARAM(),JIVariant.OPTIONAL_PARAM()});
			
			Thread.sleep(3000); //wait for 5 secs
			
	        ieObjectDispatch.callMethod("Quit");
	        JISession.destroySession(ieObjectDispatch.getAssociatedSession());
		}
		catch (JIException je)
		{
			engine.showMessage(je.getMessage());
		}
		catch(UnknownHostException uh)
		{
			engine.showMessage(uh.getMessage());
		}
		catch(InterruptedException ie)
		{
			engine.showMessage(ie.getMessage());
		}				
	...
	{
		if (event instanceof CustomUrlWindowChangedEvent)
		{
			//String eventName = ((CustomUrlWindowChangedEvent)event).getEventName();
			//For the moment we only send onunload event
