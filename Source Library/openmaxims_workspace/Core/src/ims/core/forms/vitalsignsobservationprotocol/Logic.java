...
import ims.core.forms.vitalsignsobservationprotocol.GenForm.lyrDetailLayer.tabScoreContainer.grdScoreRow;
...
public class Logic extends BaseLogic
{
	...
	private void populateConfigInstanceControls(SECSConfigurationVo voSecsConfig)
	{
		...
		if(voSecsConfig != null)
		{
			if(voSecsConfig.getTypeIsNotNull())
			{
				...
				form.chkIsRequiredForScore().setValue(voSecsConfig.getIsRequiredForScore()); //WDEV-20668
				...
				{	
					...
					for(int i=0;i<voCollRangeScores.size();i++)
					{
						...
						if(voSecsConfig.getType().equals(OBSType.TEMPERATURE))
	...
	private void enableConfigInstanceControls()
	{
		...
		form.chkIsRequiredForScore().setEnabled(true); //WDEV-20668
	}
	...
	private void populateScreenFromData(SECSVo secs)
	{
		...
		if(secs != null)
		{
			...
			form.cmbOBSProtocolType().setValue(secs.getOBSProtocolType());//WDEV-20237
			form.lblTotalScore().setValue("Score which triggers " + form.cmbOBSProtocolType().getValue()!=null ? form.cmbOBSProtocolType().getValue().getText() : "MEWS" + " Protocol:");//WDEV-20237
...
//		colRange1.setWidth(100);
		DynamicGridColumn colRange1 = form.lyrDetail().tabRange().dyngrdRangeScore().getColumns().newColumn("Greater Than Or Equal To", DYN_COLGREATERTHAN); 
		colRange1.setWidth(150); //WDEV-22399
...
//		colRange2.setWidth(100);
		DynamicGridColumn colRange2 = form.lyrDetail().tabRange().dyngrdRangeScore().getColumns().newColumn("Less Than Or Equal To", DYN_COLLESSTHAN);
		colRange2.setWidth(150); //WDEV-22399
	...
	public void populateParameterCombo( OBSTypeCollection usedItemsLookupCollection )
	{
		form.cmbParameter().clear();
		OBSTypeCollection lookupCollection = LookupHelper.getOBSType(this.domain.getLookupService());
	...
	{
		voSecs.setOBSProtocolType(form.cmbOBSProtocolType().getValue()); //WDEV-20237
	...
	private SECSConfigurationVoCollection populateConfigurationDataFromScreen(SECSConfigurationVoCollection voCollSecsConfig)
	{
		...
		if(form.cmbParameter().getValue() != null)
		{
			...
			voConfig.setIsRequiredForScore(form.chkIsRequiredForScore().getValue()); //WDEV-20668
			
			if(form.cmbParameter().getValue().equals(OBSType.TEMPERATURE) || form.cmbParameter().getValue().equals(OBSType.SYSTOLICBP) || form.cmbParameter().getValue().equals(OBSType.RESPIRATORYRATE) || form.cmbParameter().getValue().equals(OBSType.PULSE) || form.cmbParameter().getValue().equals(OBSType.OXYGENSATS) || form.cmbParameter().getValue().equals(OBSType.FILTEREDO2)) //	WDEV-12769  //WDEV-20311
			{
				...
				for(int i=0;i<form.lyrDetail().tabRange().dyngrdRangeScore().getRows().size();i++)
				{
					...
					if(form.cmbParameter().getValue().equals(OBSType.TEMPERATURE))
					{
			...
			/*	WDEV-12769else if(form.cmbParameter().getValue().equals(OBSType.OXYGENSATS))
			{
				//fill oxygen data
	...
	public void newInstance() throws ims.framework.exceptions.PresentationLogicException
	{
		if (allowNew())
		{
			...
			form.cmbOBSProtocolType().setValue(OBSProtocolType.MEWS); //WDEV-20237
		}
	}
	...
	public void updateControlsState()
	{
		...
		//WDEV-20237
		form.lblEWSProtocol().setVisible(false);
		form.txtProtocol().setEnabled(false);
		form.txtProtocol().setVisible(false);
		form.btnProtocol().setEnabled(form.getMode().equals(FormMode.EDIT));
		form.btnProtocol().setVisible(false);
	}

	private boolean areAllParametersUsed()
	{
		OBSTypeCollection collOBSType = getUsedItemsLookupCollection();
		if(collOBSType==null)
			return false;
		
		int usedItems = collOBSType.size();
...
//											errors.add("'Greater Than' value must be less than 'Less Than' value."); 
	...
	private void loadScoresGrid(OBSType type)
	{
		form.lyrDetail().tabScore().grdScore().getRows().clear();
		if(type.equals(OBSType.PATIENT_CONCERN))
		{
		...
		else if(type.equals(OBSType.CONSCIOUS_LEVEL))
		{
		...
		else if(type.equals(OBSType.URINE_OUTPUT))
		{
		...
		else if(type.equals(OBSType.ONSUPPLEMENTALOXYGEN))
		{
			YesNoCollection onSupplementalOxygen = LookupHelper.getYesNo(domain.getLookupService());
			for(int i=0; i<onSupplementalOxygen.size(); i++)
			{
				grdScoreRow row = form.lyrDetail().tabScore().grdScore().getRows().newRow();
				row.setColHidden(onSupplementalOxygen.getIndex(i));
				row.setColText(onSupplementalOxygen.getIndex(i).getText());
			}
		}
	...
	//WDEV-20237
	@Override
	protected void onCmbOBSProtocolTypeValueChanged() throws PresentationLogicException
	{
		form.lblTotalScore().setValue("Score which triggers " + form.cmbOBSProtocolType().getValue().getText()+ " Protocol:");
	}
}
