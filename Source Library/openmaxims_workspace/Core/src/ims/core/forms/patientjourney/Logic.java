...
import ims.pathways.vo.PatientJourneyClockInterfaceVo;
...
public class Logic extends BaseLogic
{
	...
	{		
		...
		switch(menuItemID)
		{
			...
			case GenForm.ContextMenus.CoreNamespace.PatientJourneyCustomControl.EditReadjustment:
				...
				break;
			//WDEV-19603
			case GenForm.ContextMenus.CoreNamespace.PatientJourneyCustomControl.ADD_COMMENT:
				addComment();
				break;
			case GenForm.ContextMenus.CoreNamespace.PatientJourneyCustomControl.ADD_EVENTCOMMENT:
				addEventComment();
				break;
			case GenForm.ContextMenus.CoreNamespace.PatientJourneyCustomControl.ADD_TARGETCOMMENT:
				addTargetComment();
				break;	
				
			case GenForm.ContextMenus.CoreNamespace.PatientJourneyCustomControl.RECORD_BREACH_REASON:
				recordBreachReason();
				break;
				
			case GenForm.ContextMenus.CoreNamespace.PatientJourneyCustomControl.REMOVE_TARGET_FROM_SCOPE:
	...
	@Override
	protected void onPatientJourneyControlSelectionChanged(Object entry) 
	{
		if (entry instanceof PatientJourneyEntry)
			setContextMenu((PatientJourneyEntry)entry);
	}
	...
	public void setValue(ims.vo.interfaces.IPatientJourney value)
	{
		...
		boolean hideAllButViewClockHistory = Boolean.TRUE.equals(form.getLocalContext().getHideAllCMExceptViewClockHistory());
		
		form.getContextMenus().Core.getPatientJourneyCustomControlAddNewEventItem().setVisible(!hideAllButViewClockHistory);
		form.getContextMenus().Core.getPatientJourneyCustomControlAsociatePathwayItem().setVisible(!hideAllButViewClockHistory);
		form.getContextMenus().Core.getPatientJourneyCustomControlViewClockHistoryItem().setVisible(true);
		form.getContextMenus().Core.getPatientJourneyCustomControlADD_COMMENTItem().setVisible(!hideAllButViewClockHistory);
		form.getContextMenus().Core.getPatientJourneyCustomControlREMOVE_TARGET_FROM_SCOPEItem().setVisible(!hideAllButViewClockHistory); // WDEV-20636
	...
	private IPatientJourneyClock[] sortClocksByStartDate(IPatientJourneyClock[] clocks) 
	{
		...
		for(int x = 0; x < clocks.length; x++)
		{
			for(int y = x + 1; y < clocks.length; y++)
			{
				...
				if(clock1.getIPatientJourneyClockStartDate() != null && clock2.getIPatientJourneyClockStartDate() != null && clock1.getIPatientJourneyClockStartDate().compareTo(clock2.getIPatientJourneyClockStartDate()) > 0)
				{					
					clocks[x] = clock2;
					clocks[y] = clock1;
				}
	...
	private IPatientJourneyClock[] extractClocks(IPatientJourney value) 
	{
		...
		for(int x = 0; x < value.getIPatientJourneyJourneyTargets().length; x++)
		{
			...
			if(clock != null && result.indexOf(clock) < 0&&!isRIEClock(clock))
			{
				result.add(clock);
			}
		}

		// WDEV-20670 - Journey clocks may exist without events or targets, so include these too
		for(int x = 0; x < value.getIPatientJourneyClockHistory().length; x++)
		{
			IPatientJourneyClock clock = value.getIPatientJourneyClockHistory()[x];
			if(clock != null && result.indexOf(clock) < 0 && !isRIEClock(clock))
			{
				result.add(clock);
			}
	...
	private boolean isRIEClock(IPatientJourneyClock clock)
	{
		if(clock!=null&&clock instanceof PatientJourneyClockInterfaceVo)
		{
			return Boolean.TRUE.equals(((PatientJourneyClockInterfaceVo)clock).getIsRIE())?true:false;
		}
		return false;
	}
...
//		 							 "<b><font color=red>&nbsp;&nbsp;Cancelled</b></font>" : "" ));
	...
	{
		if(startDate == null || targetDate == null) //WDEV-21420
			return 0;
	...
	{
		if (Boolean.TRUE.equals(form.getLocalContext().getHideAllCMExceptViewClockHistory()))
			return;
		...
		boolean isSelectedEntryEvent = false;
				...
				if (identifier != null)
				{
					...
					if(selectedEntry != null)
					{
						Identifier selectedIdentifier = (Identifier)selectedEntry.getIdentifier();
							...
							isSelectedEntryEvent = ((EventIdentifier)selectedIdentifier).getEvent() != null ? true : false;		
						
					}
				}				
			}
		}
		form.getContextMenus().Core.getPatientJourneyCustomControlADD_COMMENTItem().setVisible(true);//WDEV-19603 /WDEV-19927 	
		...
		form.getContextMenus().Core.getPatientJourneyCustomControlADD_TARGETCOMMENTItem().setVisible(isJourneyTarget); //WDEV-19603
		form.getContextMenus().Core.getPatientJourneyCustomControlADD_EVENTCOMMENTItem().setVisible(isSelectedEntryEvent);//WDEV-19603
		
		form.getContextMenus().Core.getPatientJourneyCustomControlEditReadjustmentItem().setVisible(isJourneyTarget && hasReadjustments);
		//WDEV-19849
		form.getContextMenus().Core.getPatientJourneyCustomControlRECORD_BREACH_REASONItem().setVisible(form.getLocalContext().getIPatientJourney() != null && canAddEditBreachReasons);
	}
	...
	//WDEV-19603
	public IPatientJourneyEvent getSelectedEvent()
	{
		{
			if(form.patientJourneyControl().getSelectedEntry().getIdentifier() instanceof EventIdentifier)
			{
				return  ((EventIdentifier)form.patientJourneyControl().getSelectedEntry().getIdentifier()).getEvent();
			}
		}
		return null;
	}

	public void hideAllCMExceptViewClockHistory(Boolean hide)
	{
		form.getLocalContext().setHideAllCMExceptViewClockHistory(hide);
		
	}
	
	/** WDEV-20636  **/
	public void showRemoveTargetMenuOption()
	{
		form.getContextMenus().Core.getPatientJourneyCustomControlREMOVE_TARGET_FROM_SCOPEItem().setEnabled(true);
		form.getContextMenus().Core.getPatientJourneyCustomControlREMOVE_TARGET_FROM_SCOPEItem().setVisible(true);
	}
}
