...
public class WardBayConfigDialogImpl extends BaseWardBayConfigDialogImpl
{
	...
	*/
	public void saveWardBayConfig(ims.core.vo.WardBayConfigVo wardBayConfig) throws ims.domain.exceptions.StaleObjectException, DomainInterfaceException
	{
		...
		if (wardBayConfig.getID_WardBayConfig() == null) //WDEV-20328
		{
			doWardBayConf.setWardStatus(getDomLookup(WardBayStatus.OPEN));
		}
		...
		while(it3.hasNext())
		{
			...
			if (doBayConf.getId() == null) //WDEV-20328
				doBayConf.setBayStatus(getDomLookup(WardBayStatus.OPEN));
		}
		...
		while(it.hasNext())
		{
			...
			if(doBayConf.getFloorBedSpaceLayout().isIsTemplate())
			{
				...
				while(it1.hasNext())
				{
					...
					doNewBed.setPrivateBed(doBed.isPrivateBed()); //WDEV-20516
					doNewBed.setDependency(doBed.getDependency());//WDEV-20516
