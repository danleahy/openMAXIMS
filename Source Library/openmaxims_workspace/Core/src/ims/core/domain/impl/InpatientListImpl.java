...
*/
	...
	private InpatientEpisodeVoCollection listInpatientsHibernateBySearchCriteria(PatIdType idType, String idValue, String surname, String forename, LocationRefVo hospital, LocationRefVo ward, HcpRefVo hcp,Specialty specialty, PatientRefVo patient)
	{
		...
		if (idType!=null && idValue!=null)
		{
			...
			condStr.append(andStr + " identifiers.type = :idType and " +(!isCaseSensitivePatIdSearch ? "UPPER(identifiers.value)" : "identifiers.value") + " = :idValue "); //WDEV-18817
			markers.add("idType");
			values.add(getDomLookup(idType));
			markers.add("idValue");
			
			//WDEV-22934
			if (PatIdType.NHSN.equals(idType))
				idValue = idValue.replace(" ", "");
			
			values.add(!isCaseSensitivePatIdSearch ? idValue.trim().toUpperCase() : idValue.trim()); //WDEV-18817
			andStr = " and ";
		}
		else
		{
			...
			if (hospital != null && ward == null) //WDEV-20688
			{
				LocShortMappingsVoCollection wards = listActiveWardsForHospital(hospital);
				if (wards.size() > 0)
				{	
					condStr.append(andStr + " (ip.pasEvent.location.id in " + getWardIds(wards));
					condStr.append(" OR ");
				}
				condStr.append((wards.size() > 0 ? "" : andStr) + " ip.pasEvent.location.parentLocation.id = :hospital" + (wards.size() > 0 ? ")" : ""));
	...
	private String getWardIds(LocShortMappingsVoCollection wards)
	{
