...
public class UploadDocumentImpl extends BaseUploadDocumentImpl
{
	...
	public PatientDocumentVoCollection listPatientDocuments(PatientRefVo patient, CareContextRefVo context, DocumentCategory category, CatsReferralRefVo refVo,ClinicalReferralsRefVo clinicalReferralRef, DocumentViewLevel level) 
	{		
		if(patient == null || 
		...
		//WDEV-18856
		else if (level.equals(DocumentViewLevel.CLINICALREFERRAL) )
		{
			if (clinicalReferralRef == null)
				return null;
			
			mainHql = "select pd from ClinicalReferrals as cr ";			
			mainHql +=(" left join cr.patient as p left join cr.referralDocument as pd left join pd.currentDocumentStatus as ds left join ds.correspondenceStatus as cs ");
			hql += andStr + " cr.id = :idClinicalReferral and (cs.id not in (" + DocumentStatus.DRAFT.getID() + ", " + DocumentStatus.FOR_REVIEW.getID() + ", " + DocumentStatus.FOR_UPDATE.getID() + ") or pd.currentDocumentStatus is null)  ";
			markers.add("idClinicalReferral");
			values.add(clinicalReferralRef.getID_ClinicalReferrals());
		}
