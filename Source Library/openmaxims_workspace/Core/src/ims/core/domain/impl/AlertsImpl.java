...
public class AlertsImpl extends DTODomainImplementation implements ims.core.domain.Alerts, ims.domain.impl.Transactional
{
	...
	public PatientAlert savePatientAlert(ims.core.vo.PatientAlert patientAlertItem) throws StaleObjectException, DomainInterfaceException 
	{
		...
		//WDEV-21218
		PatientAlert patAlertSaved = PatientAlertAssembler.create(doAlert);
		savePatientAlertCategories(patAlertSaved.getPatient()); 
				 
		return patAlertSaved;
	}
	...
	//WDEV-21218
	private PatientAlertCategoriesVo getPatientAlertCategories(PatientRefVo patientRef)
	{
		if(patientRef == null)
			throw new DomainRuntimeException("Cannot Get PatientAlertCategoriesVo for a null Patient ");
		
		DomainFactory factory = getDomainFactory();

		StringBuffer hql = new StringBuffer();
		hql.append("select patAlertCategories from PatientAlertCategories as patAlertCategories left join patAlertCategories.patient as patient where patient.id = :PatID ");

		List<?> list = factory.find(hql.toString(), new String[]{"PatID"}, new Object[]{patientRef.getID_Patient()});
	
		if (list!=null && list.size()>0)
		{
			return PatientAlertCategoriesVoAssembler.create((PatientAlertCategories) list.get(0));
		}
