...
import ims.pathways.forms.encounteroutcomeconfiguration.GenForm.grdResultsRow;
...
public class Logic extends BaseLogic
{
	...
	private void updateControlsState()
	{
		...
		//WDEV-19572
		form.ctnDetails().chkDisplayUndoEvent().setEnabled(FormMode.EDIT.equals(form.getMode()) && form.ctnDetails().qmbUndoEvent().getValue() != null);
	}
	@Override
	...
	{
		RTTStatusPointVoCollection coll = domain.listRTTStatusPoints(false);
	...
	{	
		...
		//WDEV-19572
		rttStatusEventVo.setUndoEvent(form.ctnDetails().qmbUndoEvent().getValue());
		rttStatusEventVo.setDisplayUndoEvent(form.ctnDetails().chkDisplayUndoEvent().getValue());
		//WDEV-19572 --- ends here
	...
	private String[] validateUIRules()
	{
		...
		if (form.ctnDetails().qmbEvent().getValue() != null && form.ctnDetails().qmbUndoEvent().getValue() != null && form.ctnDetails().qmbUndoEvent().getValue().equals(form.ctnDetails().qmbEvent().getValue()))
		{
			errors.add("'Event' and 'Undo Event' cannot be identical.");
		}
	...
	private void populateInstanceControls(RTTStatusEventMapVo selectedRecordVo)
	{
		...
		{	
			form.ctnDetails().qmbEvent().newRow(selectedRecordVo.getEvent(),selectedRecordVo.getEvent().getName());
			form.ctnDetails().qmbEvent().setValue(selectedRecordVo.getEvent());
		}
		if (selectedRecordVo.getUndoEventIsNotNull())
		{	
			form.ctnDetails().qmbUndoEvent().newRow(selectedRecordVo.getUndoEvent(),selectedRecordVo.getUndoEvent().getName());
			form.ctnDetails().qmbUndoEvent().setValue(selectedRecordVo.getUndoEvent());
		}
		form.ctnDetails().chkDisplayUndoEvent().setValue(selectedRecordVo.getDisplayUndoEvent());
		//WDEV-19572 ---- ends here
	...
	private void clearInstanceControls()
	{
		...
		form.ctnDetails().qmbUndoEvent().clear(); //WDEV-19572
		form.ctnDetails().chkDisplayUndoEvent().setValue(null); //WDEV-19572
	...
	{
		clearInstanceControls();
		form.ctnDetails().chkActive().setValue(Boolean.TRUE); //WDEV-18594
	...
	{
		form.ctnDetails().qmbEvent().clear();
		EventLiteVoCollection eventColl = getEvents(value, null);
		populateEvents(eventColl);

	}
	private EventLiteVoCollection getEvents(String value, EventRefVo eventToExclude)
	{
		EventLiteVoCollection eventColl = domain.listEvents(value, eventToExclude);
		return eventColl;
	}
	private void populateEvents(EventLiteVoCollection eventColl)
	{
		if (eventColl == null || eventColl.size() == 0) 						
		{                                               
			return;   
		}
		int eventsCount = eventColl.size();

		for (int i=0; i<eventsCount;i++)
		{
			if (eventColl.get(i) == null)
				continue;
			form.ctnDetails().qmbEvent().newRow(eventColl.get(i), eventColl.get(i).getName());
		}
	...
	//WDEV-19572 --- starts here
	@Override
	protected void onQmbUndoEventTextSubmited(String value)	throws PresentationLogicException
	{
		EventLiteVoCollection eventColl = getEvents(value, form.ctnDetails().qmbEvent().getValue());
		populateUndoEvents(eventColl);
		updateControlsState();
		
	}
	private void populateUndoEvents(EventLiteVoCollection eventColl)
	{
		form.ctnDetails().qmbUndoEvent().clear();
		...
		for (int i=0; i<eventsCount;i++)
		{
			if (eventColl.get(i) == null)
				continue;
			form.ctnDetails().qmbUndoEvent().newRow(eventColl.get(i), eventColl.get(i).getName());
		}		
		if (eventsCount == 1)
		{	
			form.ctnDetails().qmbUndoEvent().setValue(eventColl.get(0));
			form.ctnDetails().chkDisplayUndoEvent().setValue(Boolean.TRUE);
		}	
		else
			form.ctnDetails().qmbUndoEvent().showOpened();
	}
	
	@Override
	protected void onQmbUndoEventValueChanged()	throws PresentationLogicException
	{
		form.ctnDetails().chkDisplayUndoEvent().setValue(form.ctnDetails().qmbUndoEvent().getValue() != null);
		updateControlsState();	
