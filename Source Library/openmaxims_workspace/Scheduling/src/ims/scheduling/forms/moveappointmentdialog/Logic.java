...
public class Logic extends BaseLogic
{
	...
	protected void onBtnOKClick() throws ims.framework.exceptions.PresentationLogicException
	{
		...
		if(form.getLocalContext().getSaveOnDialogIsNotNull() && form.getLocalContext().getSaveOnDialog())
		{
			...
			SessionParentChildSlotRefVoCollection slots = session.calculateChildSlots(voData.getToSlot(), appointment.getCustomProcedureDuration(), appointment);
	...
	private void loadTheatreSlots()
	{
		...
		{
			//WDEV-21578 just get the session by appointment 
			SessionTheatreVo voSession = domain.getSessionForAppointment(fromSlot != null ? fromSlot.getAppointment() : null);

			if (voSession != null && voSession.getParentChildSlotsIsNotNull())
			{
	...
	private SessionTheatreVoCollection listSessions()
	{
		...
		if(voData.getFromSlotIsNotNull() && voData.getFromSlot().getAppointmentIsNotNull())
		{
			...
			searchCriteria.setFromDate(form.dteSession().getValue());
			searchCriteria.setToDate(form.dteSession().getValue());
			searchCriteria.setService(voCriteria.getSessionService());
			searchCriteria.setHospital(voCriteria.getSessionLocation());
			searchCriteria.setConsultant(voCriteria.getSessionConsultant());
			searchCriteria.setPrimaryProcedure(voCriteria.getSessionProcedure());
			searchCriteria.setTheatreType(voCriteria.getSessionTheatreType());
			searchCriteria.setConsultant(voCriteria.getSessionConsultant());
			searchCriteria.setTimeRequired(0);
			searchCriteria.setAllowOverBook(engine.hasRight(AppRight.CAN_OVERBOOK_THEATRE_APPTS));
			searchCriteria.setAnaestheticType(voCriteria.getSessionAnaestheticType());
			
			if(voCriteria != null)
			{
				voCollSessions = domain.listSession(searchCriteria);
			}
		}
