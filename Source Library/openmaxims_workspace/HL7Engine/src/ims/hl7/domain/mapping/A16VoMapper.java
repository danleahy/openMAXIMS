...
public class A16VoMapper extends VoMapper
{
	// WDEV-20445
	// This event is called when a Cancel Pending Discharge event is triggered
	
	private static final Logger	LOG	= Logger.getLogger(A16VoMapper.class);
	...
	{
		LOG.debug("A16VoMapper populateMessage: entry");
		
		ADT_A16 message = new ADT_A16();
		...
		if(event instanceof InpatientEpisodeQueueVo)
		{
			...
			if(a01Vomapper==null)
			{
				throw new HL7Exception("A16 mapper requires A01 mapper. A01 mapper not found in list of registered mappers.");			
			}
		...
		populateMSH(event.getProviderSystem(), message.getMSH(), Long.toString( new java.util.Date().getTime()), "ADT", "A16");
		message.getEVN().getEventTypeCode().setValue("A16");
