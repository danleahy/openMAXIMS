...
public class A22VoMapper extends VoMapper
{
	...
	private static final Logger	LOG	= Logger.getLogger(A22VoMapper.class); //WDEV-19481
...
//		return(a21mapper.processPatientLeave(msg, providerSystem));
	...
	{
		//WDEV-19481
		LOG.debug("A22VoMapper populateMessage: entry");
		...
		if(event instanceof InpatientEpisodeQueueVo)
		{
			...
			if(a01Vomapper==null)
			{
				throw new HL7Exception("A22 mapper requires A01 mapper. A01 mapper not found in list of registered mappers.");			
			}
...
//				message.getEVN().getEventOccurred().getTimeOfAnEvent().setValue(inpatientEpisode.getDateTimeOnHomeLeave().toString());
		...
		populateMSH(event.getProviderSystem(), message.getMSH(), Long.toString( new java.util.Date().getTime()), "ADT", "A22");
		message.getEVN().getEventTypeCode().setValue("A22");
