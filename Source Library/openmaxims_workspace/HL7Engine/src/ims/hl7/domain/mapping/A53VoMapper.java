...
public class A53VoMapper extends VoMapper
{
	...
	private static final Logger	LOG	= Logger.getLogger(A53VoMapper.class); //WDEV-19481
...
//		return(a21mapper.processPatientLeave(msg, providerSystem));
	...
	{
		LOG.debug("A53VoMapper populateMessage: entry");
		...
		if(event instanceof InpatientEpisodeQueueVo)
		{
			...
			if(a01Vomapper==null)
			{
				throw new HL7Exception("A53 mapper requires A01 mapper. A01 mapper not found in list of registered mappers.");			
			}
...
//				message.getEVN().getEventOccurred().getTimeOfAnEvent().setValue(inpatientEpisode.getDateTimeOnHomeLeave().toString());
				message.getEVN().getEventOccurred().getTimeOfAnEvent().setValue(inpatientEpisode.getDateTimeOnHomeLeave().toString(ims.framework.utils.DateTimeFormat.ISO)); //WDEV-21121
			}

		}

		populateMSH(event.getProviderSystem(), message.getMSH(), Long.toString( new java.util.Date().getTime()), "ADT", "A53");
		message.getEVN().getEventTypeCode().setValue("A53");
