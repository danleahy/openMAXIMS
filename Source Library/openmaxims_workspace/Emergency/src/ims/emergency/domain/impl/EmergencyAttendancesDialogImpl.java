...
public class EmergencyAttendancesDialogImpl extends BaseEmergencyAttendancesDialogImpl
{
	...
	public ims.emergency.vo.EpisodeDetailsVoCollection listEmergencyEpisodeByPatient(PatientRefVo patientRef, Date startDate, Date endDate)
	{
		...
		{
			String hsql1 = "select e1_1 from EmergencyEpisode as e1_1 left join e1_1.episodeOfCare as e2_1 left join e2_1.careSpell as c1_1 left join c1_1.patient as p1_1 left join e1_1.emergencyAttendances as e3_1 where e1_1.isRIE is null and p1_1.id = :idPatient order by e2_1.startDate desc, e3_1.arrivalDateTime desc"; //WDEV-18750
			...
			if (episodeDetails != null && episodeDetails.size() > 0)
				return EpisodeDetailsVoAssembler.createEpisodeDetailsVoCollectionFromEmergencyEpisode(episodeDetails);//WDEV-18750
			else
				return null;
		...
		else if(startDate != null && endDate == null)
		{
			...
			hsql += " and e3_1.arrivalDateTime > :StartDate order by e2_1.startDate desc, e3_1.arrivalDateTime desc";//WDEV-18750
			episodeDetails = factory.find(hsql, new String[] {"idPatient","StartDate"}, new Object[] {patientRef.getID_Patient(),javaStartDate});
			if(episodeDetails != null && episodeDetails.size() > 0)
			{
				return populateEpisodeAndAttendances(episodeDetails);//WDEV-18750
			}
			else
				return null;
		...
		else if(startDate == null && endDate != null)
		{
			...
			hsql += " and e3_1.arrivalDateTime < :EndDate order by e2_1.startDate desc, e3_1.arrivalDateTime desc ";//WDEV-18750
		...
		else
		{
			...
			endDate.addDay(1);
			java.util.Date javaEndDate = endDate.getDate();
			hsql += " and e3_1.arrivalDateTime between :StartDate and :EndDate order by e2_1.startDate desc, e3_1.arrivalDateTime desc ";//WDEV-18750
