...
import ims.emergency.vo.TrackingSearchCriteriaVo;
...
public class Logic extends BaseLogic
{
	...
	private static final int M_SEEN_COMNURSE		= 5040022; //WDEV-20429
	...
	private static final int M_COMMENTS			    = 5040021; //WDEV-20419
	
	//WDEV-17966
	private static final String SEEN_COMPLETE_BUTTON_TEXT 		= "+";//WDEV-20428
	private static final String SEEN_COMPLETE_NURSE_BUTTON_TEXT = "+";//WDEV-20428
	private static final String REFER_TO_BUTTON_TEXT 			= "+";//WDEV-20428
	private static final String ALLOCATE_CUBICLE_BUTTON_TEXT  	= "+"; //WDEV-20720
	private static final String MASKED_TEXT  	= "*****";
	private static final char MASKED_NAME_TEXT  	= '*';
	
	//WDEV-19389 
	private static final int VIEW_ALL							= 1;
	private static final int VIEW_MY_PATIENTS					= 2;
	private static final int VIEW_UNALOCATD						= 3;
	
	private static final int DISPLAY_IN_DEPT					= 1;
	private static final int DISPLAY_DEPARTED					= 2;
	private static final int DISPLAY_CURRENT_AND_DEPARTED		= 3;
	...
	private class TrackingPatientsComparator implements Comparator<TrackingVo> //WDEV-15819
	{
		...
		public int compare(TrackingVo o1, TrackingVo o2)
		{
			...
			*/
			...
			//wdev-19087
			else if(TrackingAreaColumn.MRN_STATUS.equals(column))
			{
				String MRN1 = getIdentifier(o1, PatIdType.MRNNUM);
				String MRN2 = getIdentifier(o2, PatIdType.MRNNUM);
				
				if (MRN1 != null && MRN2 != null)
				{
					return MRN1.toUpperCase().compareTo(MRN2.toUpperCase()) * direction;
				}
			...
			*/
			...
			{
				if(Boolean.TRUE.equals(form.getLocalContext().getED_USE_CUSTOM_ATTENDANCE_ID()))
				{
					if (o1.getAttendance() != null && o1.getAttendance().getCustomID() != null && o2.getAttendance() != null && o2.getAttendance().getCustomID() != null)
    				{
    					return o1.getAttendance().getCustomID().compareTo(o2.getAttendance().getCustomID()) * direction;
    				}
    				...
    				if (o2.getAttendance() != null && o2.getAttendance().getCustomID() != null && (o1.getAttendance() == null || o1.getAttendance().getCustomID() == null))
    				{
    					return -1 * direction;
    				}
				}
			...
			else if (TrackingAreaColumn.BED.equals(column)) //WDEV-17661
			{
				if (o1.getCurrentPartialAdmission() != null && o2.getCurrentPartialAdmission() != null)
				{
					Integer val1, val2;
					
					if (o1.getCurrentPartialAdmission().getSpecialty()!= null)
					{
						val1 = 1;
					}
					else 
					...
					if (o2.getCurrentPartialAdmission().getSpecialty() != null)
					{
						val2 = 1;
					}
					else 
			...
			else if(TrackingAreaColumn.REFERRALS.equals(column))
			{
				Integer No1 = o1.getNoOfReferrals();
				Integer No2 = o2.getNoOfReferrals();
				
				if (No1 != null && No2 != null)
				{
					return No1.compareTo(No2) * direction;
				}
			...
			else if(TrackingAreaColumn.ATTENDANCE_NOTES_PRESENT.equals(column)) //WDEV-19275
			{
				Integer No1 = o1.getNoOfAttendanceNotes();
				Integer No2 = o2.getNoOfAttendanceNotes();
			...
			else if (TrackingAreaColumn.PAWS_SCORE.equals(column))
			{
				Integer No1 = o1.getTriageDetailsIsNotNull() ? o1.getTriageDetails().getPAWSScore() : null;
				Integer No2 = o2.getTriageDetailsIsNotNull() ? o2.getTriageDetails().getPAWSScore() : null;
	...
	protected void onFormOpen(Object[] args) throws ims.framework.exceptions.PresentationLogicException
	{
		...
		if (form.getGlobalContext().Emergency.getTrackingSearchCriteriaIsNotNull())
		{
			setSearchCriteria(form.getGlobalContext().Emergency.getTrackingSearchCriteria());
		}
	}
	
	private TrackingSearchCriteriaVo getSearchCriteria()
	{
		TrackingSearchCriteriaVo searchCriteria = new TrackingSearchCriteriaVo();
		
		if (form.chkAllSites().isVisible())
			searchCriteria.setViewAllED(Boolean.TRUE.equals(form.chkAllSites().getValue()) ? true : false);
		if (form.grdTrackingCount().getValue() instanceof TrackingAreaForTrackingVo)
			searchCriteria.setTrackingArea((TrackingAreaForTrackingVo) form.grdTrackingCount().getValue());
		
		if (form.GroupView().isVisible(GroupViewEnumeration.rdoViewAll))
			searchCriteria.setViewType(getViewType());
		
		if (form.GroupDisplay().isVisible(GroupDisplayEnumeration.rdoCurrentlyInDept))
			searchCriteria.setDisplayType(getDisplayType());
		
		return searchCriteria;
	}
	
	private void setSearchCriteria(TrackingSearchCriteriaVo trackingSearchCriteriaVo) throws PresentationLogicException 
	{
		if (form.chkAllSites().isVisible())
		{
			form.chkAllSites().setValue(trackingSearchCriteriaVo.getViewAllED());
			onChkAllSitesValueChanged();
		}
		form.grdTrackingCount().setValue(trackingSearchCriteriaVo.getTrackingArea());
		onGrdTrackingCountSelectionChanged();
		
		if (form.GroupView().isVisible(GroupViewEnumeration.rdoViewAll))
		{
			setViewType(trackingSearchCriteriaVo.getViewType());
			onRadioButtonGroupViewValueChanged();
		}
		
		if (form.GroupDisplay().isVisible(GroupDisplayEnumeration.rdoCurrentlyInDept))
		{
			setDisplayType(trackingSearchCriteriaVo.getDisplayType());
	...
	private void setViewType(Integer searchType)
	{
		...
		case VIEW_MY_PATIENTS:
			form.GroupView().setValue(GroupViewEnumeration.rdoViewMyAllocated);
			break;
		case VIEW_UNALOCATD:
			form.GroupView().setValue(GroupViewEnumeration.rdoViewUnallocated);
			break;
		}				
	}
	...
	{
		GroupViewEnumeration viewType = form.GroupView().getValue();
		if (GroupViewEnumeration.rdoViewAll.equals(viewType))
		{
			return VIEW_ALL;
		}
		if (GroupViewEnumeration.rdoViewMyAllocated.equals(viewType))
		{
			return VIEW_MY_PATIENTS;
		}
		if (GroupViewEnumeration.rdoViewUnallocated.equals(viewType))
		{
			return VIEW_UNALOCATD;
		}	
		return null;
	}
	
	private void setDisplayType(Integer searchType)
	{
		if (searchType == null)
			return;
		...
		switch (searchType)
		{
		case DISPLAY_IN_DEPT:
			form.GroupDisplay().setValue(GroupDisplayEnumeration.rdoCurrentlyInDept);
			break;
		case DISPLAY_DEPARTED:
			form.GroupDisplay().setValue(GroupDisplayEnumeration.rdoDeparted);
			break;
		case DISPLAY_CURRENT_AND_DEPARTED:
			form.GroupDisplay().setValue(GroupDisplayEnumeration.rdoCurrentAndDeparted);
			break;
		}				
	}

	private Integer getDisplayType()
	{
		GroupDisplayEnumeration displayType = form.GroupDisplay().getValue();
		if (GroupDisplayEnumeration.rdoCurrentlyInDept.equals(displayType))
		{
			return DISPLAY_IN_DEPT;
		}
		if (GroupDisplayEnumeration.rdoDeparted.equals(displayType))
		{
			return DISPLAY_DEPARTED;
		}
		if (GroupDisplayEnumeration.rdoCurrentAndDeparted.equals(displayType))
		{
			return DISPLAY_CURRENT_AND_DEPARTED;
		}	
		return null;
	}
	...
	{
		copyConfigFlagsToLocalContexts();
		...
		form.getLocalContext().setSECSConfig(domain.getSECS()); //WDEV-20426
		
		form.getLocalContext().setPrimaryIDFromPASProviderSystem(domain.getPrimaryIDFromProviderSystem(Category.PAS));  //WDEV-23527
	}

	private void copyConfigFlagsToLocalContexts()
	{
		form.getLocalContext().setED_USE_CUSTOM_ATTENDANCE_ID(ConfigFlag.GEN.ED_USE_CUSTOM_ATTENDANCE_ID.getValue());
		form.getLocalContext().setED_DTA_ADMISSION_FUNCTIONALITY(ConfigFlag.GEN.ED_DTA_ADMISSION_FUNCTIONALITY.getValue());
		form.getLocalContext().setED_ALLOW_ORDERING(ConfigFlag.UI.ED_ALLOW_ORDERING.getValue());
		form.getLocalContext().setDISPLAY_PATID_TYPE(ConfigFlag.UI.DISPLAY_PATID_TYPE.getValue()); //WDEV-23527
		form.getLocalContext().setWHITEBOARD_FUNCTIONALITY(ConfigFlag.UI.WHITEBOARD_FUNCTIONALITY.getValue());
		form.getLocalContext().setAUTOMATICALLY_DISCHARGE_PATIENT_FROM_ED_ON_DNA_CALL_NUMBER(ConfigFlag.DOM.AUTOMATICALLY_DISCHARGE_PATIENT_FROM_ED_ON_DNA_CALL_NUMBER.getValue());
		form.getLocalContext().setSTALE_OBJECT_MESSAGE(ConfigFlag.UI.STALE_OBJECT_MESSAGE.getValue());
		form.getLocalContext().setVALIDATE_TRIAGE_AND_DISPLAY_PAIN_SCALE_MANDATORY_MESSAGE(ConfigFlag.UI.VALIDATE_TRIAGE_AND_DISPLAY_PAIN_SCALE_MANDATORY_MESSAGE.getValue());
		form.getLocalContext().setBED_MANAGEMENT_DASHBOARD_URL(ConfigFlag.GEN.BED_MANAGEMENT_DASHBOARD_URL.getValue());
	}
	...
	private void initializeButtons() 
	{
		...
		//WDEV-19242 
		if (form.getLocalContext().getED_DTA_ADMISSION_FUNCTIONALITY() != null && form.getLocalContext().getED_DTA_ADMISSION_FUNCTIONALITY().equals("PENDING"))
		{
			form.btnDTA().setText("  Bed Request");
			form.btnDTA().setTooltip("Bed Request");
		}
		
		form.btnComments().setImage(form.getImages().Core.Comment16);	//WDEV-20419
	...
	{
		//WDEV-22999 
		if (ConfigFlag.UI.DEMOGRAPHICS_TYPE.getValue().equals("ÜK"))
			engine.open(form.getForms().Emergency.EDWhiteBoardNewDialog, new Object[] {form.getLocalContext().getSelectedTrackingArea()});//WDEV-19012
		else
			engine.open(form.getForms().Emergency.PatientWhiteBoardEditAll, new Object[] {form.grdTrackingCount().getValue()});	
	...
	private void trackingAreaSelectionChanged() 
	{
		...
		form.getGlobalContext().Emergency.setTrackingSearchCriteria(getSearchCriteria()); //WDEV-19389 
	}
	...
	private boolean isImageColumn(DynamicGridColumn column)
	{
		...
		if(TrackingAreaColumn.REQUIRED_SIGN_OFF.equals(column.getIdentifier())) //WDEV-19293 
		{
			return true;
		}
		
		if(TrackingAreaColumn.BED.equals(column.getIdentifier())) //WDEV-19293 
		{
			return true;
		}
		
		if(TrackingAreaColumn.BED_STATUS.equals(column.getIdentifier())) //WDEV-19293 
		{
			return true;
		}
		
		if(TrackingAreaColumn.DNA.equals(column.getIdentifier())) //WDEV-19293 
		{
			return true;
		}
		
		if(TrackingAreaColumn.PRIMARY_CARE.equals(column.getIdentifier())) //WDEV-19293 
		{
			return true;
		}
		
		if(TrackingAreaColumn.NEW_RESULTS.equals(column.getIdentifier())) //WDEV-19293 
		{
			return true;
		}
	...
	private void addTrackingPatientRow(TrackingVo trackingPatient) 
	{
		...
		if (Boolean.TRUE.equals(trackingPatient.getRequiringDischargeDocumentationReview()))
		{
			row.setBackColor(Color.Orange);
//			row.setBackColor(ConfigFlag.UI.REQUIRES_DISCHARGE_DIAGNOSIS_COLOUR.getValue());
		}
		...
		for(int i=0; i<form.dyngrdTrack().getColumns().size(); i++)
		{
			...
			//wdev-16867 //WDEV-19006 
			else if( TrackingAreaColumn.TRANSFER.equals(column.getIdentifier()))
			{
				cell = row.getCells().newCell(column, DynamicCellType.IMAGE);
				if (trackingPatient.getCurrentTransferRecordIsNotNull() && !trackingPatient.getCurrentTransferRecord().getAcceptedDateTimeIsNotNull()&& !Boolean.TRUE.equals(trackingPatient.getCurrentTransferRecord().getNotAccepted()))
				{
					cell.setValue(form.getImages().Emergency.Ambulance20);
					cell.setTooltip("Decision to Transfer");
				}
				else if (trackingPatient.getCurrentTransferRecordIsNotNull() && !trackingPatient.getCurrentTransferRecord().getAcceptedDateTimeIsNotNull()&& Boolean.TRUE.equals(trackingPatient.getCurrentTransferRecord().getNotAccepted()))
				{
					cell.setValue(form.getImages().Core.Critical);
					cell.setTooltip("Transfer not accepted");
				}
				else if (trackingPatient.getCurrentTransferRecordIsNotNull() && trackingPatient.getCurrentTransferRecord().getAcceptedDateTimeIsNotNull()&& !Boolean.TRUE.equals(trackingPatient.getCurrentTransferRecord().getNotAccepted()))
				{
					cell.setValue(form.getImages().Core.AnswerBox_Yes);
					cell.setTooltip("Transfer accepted");
				}
				else
					cell.setValue(null);
			...
			{			
				if(Boolean.TRUE.equals(trackingPatient.getIsDischarged()))
				{
					...
					cell.setValue((trackingPatient.getSeenBy() != null && trackingPatient.getSeenBy().getAllocatedMedic() != null) ? trackingPatient.getSeenBy().getAllocatedMedic().toString() : null); //WDEV-16816 ////WDEV-16987 //WDEV-20429
				}
				else
				{
					cell = row.getCells().newCell(column, DynamicCellType.BUTTON);

					cell.setButtonText((trackingPatient.getSeenBy() != null && trackingPatient.getSeenBy().getAllocatedMedic() != null) ? trackingPatient.getSeenBy().getAllocatedMedic().toString() : SEEN_COMPLETE_BUTTON_TEXT); 
					cell.setTooltip((trackingPatient.getSeenBy() != null && trackingPatient.getSeenBy().getAllocatedMedic() != null) ? trackingPatient.getSeenBy().getAllocatedMedic().toString() : SEEN_COMPLETE_BUTTON_TEXT); //WDEV-16987 //WDEV-17966 //WDEV-20429
				}
			}
			else if(TrackingAreaColumn.SEEN_BY_HCP_ALLOCATED_DATE_TIME.equals(column.getIdentifier()))
			{
				...
				cell.setTooltip((trackingPatient.getSeenBy() != null && trackingPatient.getSeenBy().getAllocatedDateTime() != null) ? trackingPatient.getSeenBy().getAllocatedDateTime().toString()  : null); //WDEV-20429
			}
			else if(TrackingAreaColumn.SEEN_BY_HCP_SEEN_DATE_TIME.equals(column.getIdentifier()))
			{
				...
				cell.setTooltip((trackingPatient.getSeenBy() != null && trackingPatient.getSeenBy().getSeenDateTime() != null) ? trackingPatient.getSeenBy().getSeenDateTime().toString()  : null); //WDEV-20429
			}
			//WDEV-20429
			else if(TrackingAreaColumn.SEEN_BY_NURSE.equals(column.getIdentifier()))
			{			
				if(Boolean.TRUE.equals(trackingPatient.getIsDischarged()))
				{
					cell = row.getCells().newCell(column, DynamicCellType.STRING);
					cell.setValue((trackingPatient.getSeenByNurse() != null && trackingPatient.getSeenByNurse().getAllocatedNurse() != null) ? trackingPatient.getSeenByNurse().getAllocatedNurse().toString()  : null); 
				}
				else
				{
					cell = row.getCells().newCell(column, DynamicCellType.BUTTON);

					cell.setButtonText((trackingPatient.getSeenByNurse() != null && trackingPatient.getSeenByNurse().getAllocatedNurse() != null) ? trackingPatient.getSeenByNurse().getAllocatedNurse().toString() : SEEN_COMPLETE_NURSE_BUTTON_TEXT); 
					cell.setTooltip((trackingPatient.getSeenByNurse() != null && trackingPatient.getSeenByNurse().getAllocatedNurse() != null) ? trackingPatient.getSeenByNurse().getAllocatedNurse().toString() : SEEN_COMPLETE_NURSE_BUTTON_TEXT); 
				}
			}
			else if(TrackingAreaColumn.SEEN_BY_NURSE_ALLOCATED_DATE_TIME.equals(column.getIdentifier()))
			{
				cell = row.getCells().newCell(column, DynamicCellType.STRING);
				cell.setValue((trackingPatient.getSeenByNurse() != null && trackingPatient.getSeenByNurse().getAllocatedDateTime() != null) ? trackingPatient.getSeenByNurse().getAllocatedDateTime().toString()  : null);
				cell.setTooltip((trackingPatient.getSeenByNurse() != null && trackingPatient.getSeenByNurse().getAllocatedDateTime() != null) ? trackingPatient.getSeenByNurse().getAllocatedDateTime().toString()  : null);
				
			}
			else if(TrackingAreaColumn.SEEN_BY_NURSE_SEEN_DATE_TIME.equals(column.getIdentifier()))
			{
				cell = row.getCells().newCell(column, DynamicCellType.STRING);
				cell.setValue((trackingPatient.getSeenByNurse() != null && trackingPatient.getSeenByNurse().getSeenDateTime() != null) ? trackingPatient.getSeenByNurse().getSeenDateTime().toString()  : null);
				cell.setTooltip((trackingPatient.getSeenByNurse() != null && trackingPatient.getSeenByNurse().getSeenDateTime() != null) ? trackingPatient.getSeenByNurse().getSeenDateTime().toString()  : null);
			}
			...
			*/
			...
			else if(TrackingAreaColumn.ATTENDANCE_ID.equals(column.getIdentifier()))
			{
				...
				{
					cell = row.getCells().newCell(column, DynamicCellType.STRING);
					cell.setValue(trackingPatient.getAttendance() != null ? trackingPatient.getAttendance().getCustomID(): null);
				}
				else
				{
			...
			//WDEV-17661
			else if(TrackingAreaColumn.BED.equals(column.getIdentifier()))
			{
				cell = row.getCells().newCell(column, DynamicCellType.IMAGE);
				cell.setValue(getBedImage(trackingPatient.getCurrentPartialAdmission()));//WDEV-23199
				cell.setTooltip(getCurrentPartialAdmissionTooltip(trackingPatient.getCurrentPartialAdmission()));
			}
			else if(TrackingAreaColumn.BED_STATUS.equals(column.getIdentifier()))
			{
				...
				cell.setTooltip(getBedStatusTooltip(trackingPatient.getAssociatedPendingEmergencyAdmission(), trackingPatient.getBedAvailability()));
			}
			...
			else if(TrackingAreaColumn.COMMENTS.equals(column.getIdentifier()))
			{
				...
				cell.setTooltip(trackingPatient.getAttendance() != null ? trackingPatient.getAttendance().getComments() : null);//WDEV-20419
			}
			...
			else if(TrackingAreaColumn.HOSPITAL_NUMBER.equals(column.getIdentifier()))
			{
				...
				cell = row.getCells().newCell(column, DynamicCellType.STRING);
				String identifier = getIdentifier(trackingPatient, PatIdType.HOSPNUM);
				...
				//WDEV-17966
				if(noOfHospNumber != null && noOfHospNumber > 1 && !Boolean.TRUE.equals(form.getGlobalContext().Emergency.getTrackingPublicArea()))
				{
				...
				else if (maskInPublicArea(column) && Boolean.TRUE.equals(form.getGlobalContext().Emergency.getTrackingPublicArea()) && tableCell.getValue()!=null )
				{
					tableCell.setValue(MASKED_TEXT);
					tableCell.setTooltip(null);
				}*/
				cell.setValue(identifier);
				
				StringBuffer tooltip = new StringBuffer();
				tooltip.append(identifier);
				
				if(noOfHospNumber != null && noOfHospNumber > 1 && !Boolean.TRUE.equals(form.getGlobalContext().Emergency.getTrackingPublicArea()))
				{
					tooltip.append("<br>");
					tooltip.append("Patient has multiple Hospital Numbers");
				}
			...
			else if(TrackingAreaColumn.LENGTH_OF_STAY.equals(column.getIdentifier()))
			{
				...
				*/
				String los = trackingPatient.getAttendance() != null ? EmergencyDisplayHelper.minutesToHoursAndMinutes(trackingPatient.getAttendance().getLengthOfStay()) : null;
				cell.setValue(los);
				cell.setTooltip(los);
				setLOSColorCell(cell, trackingPatient);
			}
			else if(TrackingAreaColumn.NHS_NUMBER.equals(column.getIdentifier()))
			{
				...
				//WDEV-17966
				if(noOfNHSNumber != null && noOfNHSNumber > 1 && !Boolean.TRUE.equals(form.getGlobalContext().Emergency.getTrackingPublicArea()))//WDEV-17966
				{
					tableCell = tableRow.getCells().newCell();
					tableCell.setType(TableCellType.IMAGE);		
					tableCell.setValue(form.getImages().Core.InfoEnabled16);
				...
				*/
				String iden = getIdentifier(trackingPatient, PatIdType.NHSN);
				cell.setValue(iden);
				StringBuffer tooltip = new StringBuffer();
				tooltip.append(iden);
				
				if(noOfNHSNumber != null && noOfNHSNumber > 1 && !Boolean.TRUE.equals(form.getGlobalContext().Emergency.getTrackingPublicArea()))
				{
					tooltip.append("<br>");
					tooltip.append("Patient has multiple NHS Numbers");
				}
			...
			{
				cell = row.getCells().newCell(column, DynamicCellType.STRING);
				cell.setValue(trackingPatient.getPatient() != null ? trackingPatient.getPatient().calculateAgeText() : null);//wdev-19641
				
				//WDEV-20418
				if (trackingPatient.getPatient()!=null && trackingPatient.getPatient().calculateAgeText()!=null && Sex.MALE.equals(trackingPatient.getPatient().getSex()))
				{
					cell.setBackColor(Color.LightBlue);
				}
				else if (trackingPatient.getPatient()!=null && trackingPatient.getPatient().calculateAgeText()!=null && Sex.FEMALE.equals(trackingPatient.getPatient().getSex()))
				{
					cell.setBackColor(Color.LightPink);
				}
			}
			...
			else if(TrackingAreaColumn.PATIENT_FORENAME.equals(column.getIdentifier()))
			{
				...
				if (maskInPublicArea(column) && Boolean.TRUE.equals(form.getGlobalContext().Emergency.getTrackingPublicArea()) && tableCell.getValue()!=null )
				{
					tableCell.setValue((trackingPatient.getPatient() != null && trackingPatient.getPatient().getName() != null) ? getMaskText(trackingPatient.getPatient().getName().getForename()) : null);
					tableCell.setTooltip(null);
				}
				
				setNameKPIColorCell(cellOptions, trackingPatient, false);
				*/
				cell.setValue((trackingPatient.getPatient() != null && trackingPatient.getPatient().getName() != null) ? trackingPatient.getPatient().getName().getForename() : null);
				cell.setTooltip((trackingPatient.getPatient() != null && trackingPatient.getPatient().getName() != null) ? trackingPatient.getPatient().getName().getForename() : null);
				setNameKPIColorCell(cell, trackingPatient);
			}
			else if(TrackingAreaColumn.PATIENT_GENDER.equals(column.getIdentifier()))
			{
				...
				//WDEV-20418
				if (Sex.MALE.equals(trackingPatient.getPatient().getSex()))
				{
					cell.setBackColor(Color.LightBlue);
				}
				else if (Sex.FEMALE.equals(trackingPatient.getPatient().getSex()))
				{
					cell.setBackColor(Color.LightPink);
				}
			}
			else if(TrackingAreaColumn.PATIENT_SURNAME.equals(column.getIdentifier()))
			{
				...
				if (maskInPublicArea(column) && Boolean.TRUE.equals(form.getGlobalContext().Emergency.getTrackingPublicArea()) && tableCell.getValue()!=null )
				{
					tableCell.setValue((trackingPatient.getPatient() != null && trackingPatient.getPatient().getName() != null) ? getMaskText(trackingPatient.getPatient().getName().getSurname()) : null);
					tableCell.setTooltip(null);
				}
				
				setNameKPIColorCell(cellOptions, trackingPatient, false);
				*/
				cell.setValue((trackingPatient.getPatient() != null && trackingPatient.getPatient().getName() != null) ? trackingPatient.getPatient().getName().getSurname() : null);
				cell.setTooltip((trackingPatient.getPatient() != null && trackingPatient.getPatient().getName() != null) ? trackingPatient.getPatient().getName().getSurname() : null);
				setNameKPIColorCell(cell, trackingPatient);
			}
			...
			else if(TrackingAreaColumn.TRIAGE_PRIORITY.equals(column.getIdentifier()))
			{
				...
				*/
				cell.setValue((trackingPatient.getTriageDetails() != null && trackingPatient.getTriageDetails().getCurrentTriagePriority() != null) ? "<b>" + trackingPatient.getTriageDetails().getCurrentTriagePriority().getText() + "</b>" : null);
				cell.setTooltip((trackingPatient.getTriageDetails() != null && trackingPatient.getTriageDetails().getCurrentTriagePriority() != null) ? "<b>" + trackingPatient.getTriageDetails().getCurrentTriagePriority().getText() + "</b>" : null);
				setPriorityCellColor(cell, trackingPatient);
			}
			...
			else if(TrackingAreaColumn.REFERRALS.equals(column.getIdentifier()))
			{
				cell = row.getCells().newCell(column, DynamicCellType.INT);
				cell.setValue(trackingPatient.getNoOfReferrals());
			}
			//wdev-19087
			else if(TrackingAreaColumn.MRN_STATUS.equals(column.getIdentifier()))
			{
				cell = row.getCells().newCell(column, DynamicCellType.STRING);
				
				if( isNotValidMRN(trackingPatient, PatIdType.MRNNUM) == true)
				{
					if( trackingPatient.getPatientIsNotNull() && trackingPatient.getPatient().getMRNStatusIsNotNull())
						cell.setValue(trackingPatient.getPatient().getMRNStatus().getIItemText());
				}
				
			}
			//WDEV-19275
			else if (TrackingAreaColumn.ATTENDANCE_NOTES_PRESENT.equals(column.getIdentifier()))
			{
				cell = row.getCells().newCell(column, DynamicCellType.IMAGE);
				Integer noteCount = trackingPatient.getNoOfAttendanceNotes();
				
				if (noteCount != null && noteCount.intValue() > 0)
				{
					cell.setValue(form.getImages().Emergency.CommentNotes16x16);
					cell.setTooltip(String.valueOf(noteCount) + " attendance note" + (noteCount.intValue() > 1 ? "s" :"") + " recorded");
				}				
			}
			else if (TrackingAreaColumn.REQUIRED_SIGN_OFF.equals(column.getIdentifier())) //WDEV-19293
			{
				cell = row.getCells().newCell(column, DynamicCellType.IMAGE);
				
				if (trackingPatient.getAttendance().getRequiresSignOff()!=null && (RequiresSignOff.REQUIRES_SIGN_OFF.equals(trackingPatient.getAttendance().getRequiresSignOff()) || RequiresSignOff.SIGNED_OFF.equals(trackingPatient.getAttendance().getRequiresSignOff())))
				{
					cell.setValue(trackingPatient.getAttendance().getRequiresSignOff().getImage());
					cell.setTooltip(trackingPatient.getAttendance().getRequiresSignOff().getText());
				}				
			}
			else if (TrackingAreaColumn.OBS_SCORE.equals(column.getIdentifier())) //WDEV-20426
			{
				if (form.getLocalContext().getSECSConfigIsNotNull())
				{	
    				cell = row.getCells().newCell(column, DynamicCellType.STRING);
    				cell.setValue(trackingPatient.getTriageDetailsIsNotNull() && trackingPatient.getTriageDetails().getOBSScoreIsNotNull() ? trackingPatient.getTriageDetails().getOBSScore().toString() : null);
    				cell.setTooltip(trackingPatient.getTriageDetailsIsNotNull() && trackingPatient.getTriageDetails().getOBSScoreIsNotNull()? trackingPatient.getTriageDetails().getOBSScore().toString() : null);
				}			
			}
			else if (TrackingAreaColumn.PAWS_SCORE.equals(column.getIdentifier())) //WDEV-21205
			{
   				cell = row.getCells().newCell(column, DynamicCellType.STRING);
   				cell.setValue(trackingPatient.getTriageDetailsIsNotNull() && trackingPatient.getTriageDetails().getPAWSScoreIsNotNull() ? trackingPatient.getTriageDetails().getPAWSScore().toString() : null);
   				cell.setTooltip(trackingPatient.getTriageDetailsIsNotNull() && trackingPatient.getTriageDetails().getPAWSScoreIsNotNull()? trackingPatient.getTriageDetails().getPAWSScore().toString() : null);
			}
			...
			{
				if(!DynamicCellType.STRING.equals(cell.getType()))
				{
					cell = row.getCells().newCell(column, DynamicCellType.STRING);
				}
	...
	private void setPriorityCellColor(DynamicGridCell cell, TrackingVo tracking) 
	{
		if(cell == null)
			return;
		
		Color priorityFontColor = null;
		Color priorityBackgroundColor = null;
		...
		{
			TriagePriorityKpConfigVo triagePriorityKpConfig = getTriagePriorityKpConfig(tracking.getTriageDetails().getCurrentTriagePriority());				//wdev-19349
			//WDEV-20583 starts here MAXIMS_10.1.2.0 
			if (tracking.getTriageDetails().getMedicInterventionStartDateTime() != null)
			{				
				if ((triagePriorityKpConfig == null || (triagePriorityKpConfig.getDefaultRowColour() == null && triagePriorityKpConfig.getDefaultFontColour() == null)) && tracking.getTriageDetails().getCurrentTriagePriority() != null && tracking.getTriageDetails().getCurrentTriagePriority().getColor() != null)
				{		
					priorityFontColor = tracking.getTriageDetails().getCurrentTriagePriority().getColor();										
				}
				if (triagePriorityKpConfig != null && triagePriorityKpConfig.getDefaultFontColour() != null)
					priorityFontColor = triagePriorityKpConfig.getDefaultFontColour();
				if (triagePriorityKpConfig != null && triagePriorityKpConfig.getDefaultRowColour() != null)			//wdev-19349
					priorityBackgroundColor = triagePriorityKpConfig.getDefaultRowColour();
			}
			//WDEV-20583 end MAXIMS_10.1.2.0 
			else
			{
					...
					{	
						if(triagePriorityKpConfig.getPriorityBreachWarningKPI() != null && triagePriorityKpConfig.getPriorityBreachedKPI() != null)
						{
							if(timeSinceTriage >= triagePriorityKpConfig.getPriorityBreachWarningKPI() && timeSinceTriage < triagePriorityKpConfig.getPriorityBreachedKPI())
							{
								...
								priorityBackgroundColor = triagePriorityKpConfig.getPriorityBreachWarningKPIBackgroundColour();	
							...
							else if(timeSinceTriage >= triagePriorityKpConfig.getPriorityBreachedKPI())
							{
								...
								priorityBackgroundColor = triagePriorityKpConfig.getPriorityBreachedKPIBackgroundColour();
		...
		if(priorityBackgroundColor != null)
			cell.setBackColor(priorityBackgroundColor);	
		
		if(priorityFontColor != null)
			cell.setTextColor(priorityFontColor);	
		...
		{
			if(priorityFlashingTextColour != null)
				cell.setTextColor(priorityFlashingTextColour);	
		}
	}
	
	private void setNameKPIColorCell(DynamicGridCell cell,	TrackingVo trackingPatient) 
	{
		if(cell == null || trackingPatient == null)
			return;
		...
		if(surnameBackgroudColor != null)
			cell.setBackColor(surnameBackgroudColor);	
		
		if(surnameFontColor != null)
			cell.setTextColor(surnameFontColor);	
		...
		{
			if(surnameFlashingTextColour != null)
				cell.setTextColor(surnameFlashingTextColour);
		}
	}

	
	private void setLOSColorCell(DynamicGridCell cell,	TrackingVo trackingPatient) 
	{
		if(cell == null || trackingPatient == null)
			return;
		...
		if(losBackgroudColor != null)
			cell.setBackColor(losBackgroudColor);	
		if(losFontColor != null)
			cell.setTextColor(losFontColor);	
		...
		{
			if(losFlashingTextColour != null)
				cell.setTextColor(losFlashingTextColour);	
		}
	}

	//WDEV-23199
	private Image getBedImage(EDPartialAdmissionForTrackingVo partialAdmission)
	{
		if(partialAdmission == null)
			return null;
		
		if(form.getLocalContext().getED_DTA_ADMISSION_FUNCTIONALITY() != null && form.getLocalContext().getED_DTA_ADMISSION_FUNCTIONALITY().equals("PARTIAL") && partialAdmission.getSpecialtyIsNotNull())
		{
			if(partialAdmission.getAllocatedDateTime() != null)
			{
	...
	//WDEV-17661
	private String getCurrentPartialAdmissionTooltip(EDPartialAdmissionForTrackingVo currentPartialAdmission)
	{
		...
		if (currentPartialAdmission.getSpecialtyIsNotNull())
			str.append("<b>Specialty: </b>" + currentPartialAdmission.getSpecialty().getText() + "<br>");
		
		if (currentPartialAdmission.getAdmittingConsultantIsNotNull())
			str.append("<b>Admitting Consultant: </b>" + currentPartialAdmission.getAdmittingConsultant().getName());

		if (currentPartialAdmission.getAdmissionWardIsNotNull())
			str.append("<br>" + "<b>Admitting Ward: </b>" + currentPartialAdmission.getAdmissionWard().getName());
		else if (currentPartialAdmission.getAllocatedWardIsNotNull())
			str.append("<br>" + "<b>Allocated Ward: </b>" + currentPartialAdmission.getAllocatedWard().getName());
	...
	private String getBedStatusTooltip(PendingEmergencyAdmissionForTrackingFormVo pendingEmergencyAdmission, BedAvailabilityForTrackingVo bedAvailability) 
	{
		StringBuffer sb = new StringBuffer(); //wdev-19640
	...
	private void setPriorityCellColor(TableCellOptions cellOptions, TrackingVo tracking) 
	{
		...
		if((priorityBreachWarning && YesNo.YES.equals(priorityBreachWarningFlashing))
				|| (priorityBreached && YesNo.YES.equals(priorityBreachedFlashing)))
		{
			if(priorityFontColor != null && priorityFlashingTextColour != null && priorityBackgroundColor != null)
				cellOptions.setTextAndBackgroundBlink(priorityFontColor, priorityFlashingTextColour, priorityBackgroundColor, priorityBackgroundColor);	
		}
		
		if(priorityBackgroundColor != null)
			cellOptions.setBackgroundColor(priorityBackgroundColor);	
	...
	{
		form.getTimers().getAreaCountRefrresh().setEnabled(false);
	...
	private void updateControlsState()
	{
		...
		boolean useOCSFunctionality = Boolean.TRUE.equals(form.getLocalContext().getED_ALLOW_ORDERING()); //WDEV-19231
		
		boolean hasPatientSeenByMedic = ((form.getLocalContext().getSelectedTrackingPatient() != null 
				&& form.getLocalContext().getSelectedTrackingPatient().getSeenBy() != null 
				&& form.getLocalContext().getSelectedTrackingPatient().getSeenBy().getAllocatedMedicIsNotNull()) ? true : false); //WDEV-20088
		
		form.btnEventHistory().setEnabled(isPatientSelected);
		form.btnViewHistory().setEnabled(isPatientSelected);
		
		form.btnResult().setVisible(useOCSFunctionality); //WDEV-19231
		...
		//WDEV-23527
		PatIdType dispIdType = PatIdType.getNegativeInstance(form.getLocalContext().getDISPLAY_PATID_TYPE());
		PatIdType primaryIDFromPASroviderSystem = form.getLocalContext().getPrimaryIDFromPASProviderSystem();
		PatientShort patientSelected = form.getGlobalContext().Core.getPatientShort();
		boolean isValidPatIDForAdmission = PatIdType.NHSN.equals(dispIdType)? (PatIdType.NHSN.equals(primaryIDFromPASroviderSystem) ? false : patientSelected!=null && primaryIDFromPASroviderSystem!=null && patientSelected.getPatId(primaryIDFromPASroviderSystem) != null) 
									: (patientSelected!=null && patientSelected.getPatId(dispIdType) != null);
		...
		form.btnMove().setEnabled(isPatientSelected && !isDischarged && isCurrentLoggedInUserMemberOfStaff); //WDEV-18893
		form.btnMove().setText(hasPatientSeenByMedic ? " Save + Move" : " Move "); //WDEV-20088
		form.btnDNWAdd().setEnabled(isPatientSelected && !isDischarged);//WDEV-17283
		
		form.btnOrder().setVisible(useOCSFunctionality); //WDEV-19231
		...
		form.btnDTA().setEnabled(isPatientSelected && !isDischarged && engine.hasRight(AppRight.ED_CAN_PERFORM_ADMISSION) && isValidPatIDForAdmission);//WDEV-16987 //WDEV-21171  WDEV-23527
		...
		form.getContextMenus().Emergency.getTrackingTriageMenuMOVEItem().setVisible(isPatientSelected && !isDischarged && isCurrentLoggedInUserMemberOfStaff); //WDEV-18893
		...
		form.getContextMenus().Emergency.getTrackingTriageMenuORDERItem().setVisible(isPatientSelected && !isDischarged && useOCSFunctionality); //WDEV-19231 
		form.getContextMenus().Emergency.getTrackingTriageMenuRESULTItem().setVisible(isPatientSelected && useOCSFunctionality); //WDEV-19231 
		...
		//WDEV-21171 new condition for quick registration patients
		form.getContextMenus().Emergency.getTrackingTriageMenuDTAItem().setVisible(isPatientSelected && !isDischarged && engine.hasRight(AppRight.ED_CAN_PERFORM_ADMISSION) && isValidPatIDForAdmission && !(form.getLocalContext().getED_DTA_ADMISSION_FUNCTIONALITY() != null && form.getLocalContext().getED_DTA_ADMISSION_FUNCTIONALITY().equals("PENDING")));//WDEV-19242 WDEV-23527 
		form.getContextMenus().Emergency.getTrackingTriageMenuBED_REQUESTItem().setVisible(isPatientSelected && !isDischarged && engine.hasRight(AppRight.ED_CAN_PERFORM_ADMISSION) && isValidPatIDForAdmission && form.getLocalContext().getED_DTA_ADMISSION_FUNCTIONALITY() != null && form.getLocalContext().getED_DTA_ADMISSION_FUNCTIONALITY().equals("PENDING"));//WDEV-19242 WDEV-23527
		
		form.getContextMenus().Emergency.getTrackingTriageMenuVIEW_TRIAGEItem().setVisible(isPatientSelected && isDischarged);//WDEV-17153
		
		form.getContextMenus().Emergency.getTrackingTriageMenuCOMMENTSItem().setVisible(isPatientSelected && !isDischarged);//WDEV-20419
		form.getContextMenus().Emergency.getTrackingTriageMenuSEEN_COMPLETE_NURSEItem().setVisible(isPatientSelected && !isDischarged);//WDEV-20429
		
		
		form.btnRefresh().setVisible(true);
		form.btnRefresh().setEnabled(true);		
		
		form.btnWhiteBoard().setVisible(Boolean.TRUE.equals(form.getLocalContext().getWHITEBOARD_FUNCTIONALITY()));//WDEV-17922
		
		form.btnComments().setEnabled(isPatientSelected && !isDischarged); //WDEV-20419
		form.btnSeenCompleteNurse().setEnabled(isPatientSelected && (!isDischarged || (isDischarged && form.getGlobalContext().Core.getPatientShort().getWardIsNotNull()))); //WDEV-20429
		
		form.btnBedManagementDashboard().setVisible(form.grdTrackingCount().getValue() instanceof TrackingAreaForTrackingVo);
		
		//WDEV-23635
		boolean showEdDashboardButton = ConfigFlag.GEN.ED_DASHBOARD_URL.getValue() != null && ConfigFlag.GEN.ED_DASHBOARD_URL.getValue().length() > 0;
		form.btnEdDashboard().setVisible(showEdDashboardButton);
	...
	private void showHidebuttons(Integer menuaction)
	{
		...
		else if( M_SEEN_COMNURSE == menuaction ) //WDEV-20429
			form.btnSeenCompleteNurse().setVisible(false);
		...
		else if (M_COMMENTS == menuaction) //WDEV-20419
		{
			form.btnComments().setVisible(false);
		}
	}
	//wdev-16751
	...
	private void setContext(TrackingVo tracking) 
	{
		...
		else if(tracking.getEpisode() != null && tracking.getEpisode().getEpisodeOfCare() != null)
		{
			form.getGlobalContext().Core.setEpisodeofCareShort(domain.getEpisodeOfCare(tracking.getEpisode().getEpisodeOfCare()));
		}
		else if(tracking.getPatient() != null)
		{
			PatientShort patientShort = new PatientShort(tracking.getPatient().getID_Patient(), tracking.getPatient().getVersion_Patient());
			form.getGlobalContext().Core.setPatientShort(patientShort);
		}
	...
	private boolean updateDNWStatus(int calledAmountIncrease, boolean showConfirmationMessageBox) 
	{
		if(!(form.dyngrdTrack().getValue() instanceof TrackingVo))
			return false;
		...
		if(tempTrackingVo != null && tempTrackingVo.getVersion_Tracking() != trackingWithDNW.getVersion_Tracking() && trackingWithDNW.getCurrentDNWIsNotNull() && form.getLocalContext().getAUTOMATICALLY_DISCHARGE_PATIENT_FROM_ED_ON_DNA_CALL_NUMBER() != null && trackingWithDNW.getCurrentDNW().getCurrentStatus().getCalledAmount() == form.getLocalContext().getAUTOMATICALLY_DISCHARGE_PATIENT_FROM_ED_ON_DNA_CALL_NUMBER().intValue()) //WDEV-18386 - NPE on this line on site
		{
			engine.showMessage(form.getLocalContext().getSTALE_OBJECT_MESSAGE());
			return true;
		}
		//---------
		trackingWithDNW.setCurrentDNW(populateDNWRecord(trackingWithDNW.getCurrentDNW(), calledAmountIncrease));
		
		Boolean sendHL7Discharge = null;	//wdev-19268
		if(form.getLocalContext().getAUTOMATICALLY_DISCHARGE_PATIENT_FROM_ED_ON_DNA_CALL_NUMBER() != null && trackingWithDNW.getCurrentDNW().getCurrentStatus().getCalledAmount() == form.getLocalContext().getAUTOMATICALLY_DISCHARGE_PATIENT_FROM_ED_ON_DNA_CALL_NUMBER().intValue()) //wdev-15897
		{
			if(showConfirmationMessageBox)//WDEV-23019
			{
				form.getLocalContext().setDTAPlusDischargeMessageBox(engine.showMessage("Are you sure you would like to discharge this patient?" , "Confirm Discharge", MessageButtons.YESNO, MessageIcon.QUESTION));
				return false;
			}
			
			trackingWithDNW = dischargePatient(trackingWithDNW);
			sendHL7Discharge = Boolean.TRUE;			//wdev-19268
		}
		...
		try 
		{
			domain.saveTracking(trackingWithDNW,sendHL7Discharge);	//wdev-19268
		} 
	...
	protected void onChkAllSitesValueChanged() throws PresentationLogicException 
	{
		...
		if (form.grdTrackingCount().getValue() != null)
			form.getGlobalContext().Emergency.setTrackingSearchCriteria(getSearchCriteria()); //WDEV-19389 
...
//			}
	...
	private void saveTrackingWithWaitingToBeSeenStatus()
	{
		...
		TrackingSaveVo tracking = domain.getTracking(form.getLocalContext().getSelectedTrackingPatient());
		
		if(tracking == null)
			return;
		
		if( tracking.getTriageDetails() != null && tracking.getCurrentArea() != null)
			tracking.setCurrentStatus(createTrackingStatus(TrackingStatus.WAITING_TO_BE_SEEN_BY_A_MEDIC));
		
		if( tracking.getTriageDetails() != null && form.getGlobalContext().Emergency.getTriageCompletedDateTime() != null)
		...
		try
		{
			domain.saveTracking(tracking, null);
		}
		catch (StaleObjectException e)
		{
			engine.showMessage(form.getLocalContext().getSTALE_OBJECT_MESSAGE());
	...
	protected void onContextMenuItemClick(int menuItemID, Control sender) throws PresentationLogicException
	{
		...
		switch (menuItemID)
		{
			...
			case ims.emergency.forms.tracking.GenForm.ContextMenus.EmergencyNamespace.TrackingTriageMenu.DTA :
				onBtnDTAClick();
			break;
			case ims.emergency.forms.tracking.GenForm.ContextMenus.EmergencyNamespace.TrackingTriageMenu.BED_REQUEST : //WDEV-19242 
				onBtnDTAClick();
			break;
			...
			case ims.emergency.forms.tracking.GenForm.ContextMenus.EmergencyNamespace.TrackingTriageMenu.DNW_MINUS :
				onBtnDNWMinusClick();
			break;	
			case ims.emergency.forms.tracking.GenForm.ContextMenus.EmergencyNamespace.TrackingTriageMenu.COMMENTS : //WDEV-20419
				onBtnCommentsClick();
			break;	
			
			case ims.emergency.forms.tracking.GenForm.ContextMenus.EmergencyNamespace.TrackingTriageMenu.SEEN_COMPLETE_NURSE : //WDEV-20429
				onBtnSeenCompleteNurseClick();
			break;
		}
	...
	private void openEDAssessmentDialog()
	{
		...
		//wdev-17309
		if( Boolean.TRUE.equals(domain.wasTriageAlreadyCreated(form.getLocalContext().getSelectedTrackingPatient())))
			engine.open(form.getForms().Emergency.EDAssessmentDialog, new Object[] {DISPLAY_CLINICIANASSESSMNRWORKLIST,form.getLocalContext().getSelectedTrackingPatient()},"Clinician Assessment Worklist");
		else
			engine.open(form.getForms().Emergency.EDAssessmentDialog, new Object[] {DISPLAY_TRIAGE_FROM_TRACKING,form.getLocalContext().getSelectedTrackingPatient()},"Triage");		//wdev-17405
	}
	...
	protected void onBtnDTAClick() throws PresentationLogicException
	{
		...
		else if (form.getLocalContext().getED_DTA_ADMISSION_FUNCTIONALITY() != null && form.getLocalContext().getED_DTA_ADMISSION_FUNCTIONALITY().equals("PARTIAL")) 
		{
	...
	protected void onMessageBoxClosed(int messageBoxId, DialogResult result) throws PresentationLogicException
	{
		if( form.getLocalContext().getDTAWithTransferMessageId() != null && messageBoxId == form.getLocalContext().getDTAWithTransferMessageId())
		{
			...
			{
				//WDEV-19006 
				if (form.getLocalContext().getPartialAdmissionForTrackingVo() != null && form.getLocalContext().getPartialAdmissionForTrackingVo().getCurrentPartialAdmission() != null)
				{
					TrackingWithPartialAdmissionVo trackingwithPartialAdmission = (TrackingWithPartialAdmissionVo) form.getLocalContext().getPartialAdmissionForTrackingVo().clone();
					EDPartialAdmissionVo partialAdmission = (EDPartialAdmissionVo) trackingwithPartialAdmission.getCurrentPartialAdmission().clone();
					if (trackingwithPartialAdmission != null)
					{
						if(trackingwithPartialAdmission.getCurrentPartialAdmission() != null)
						{
							// update the Partial Admission record
							partialAdmission.setAllocatedStatus(AllocationStatus.CANCELLED_ADMISSION);
						}
						
						String[] errors1 = partialAdmission.validate();
						...
						trackingwithPartialAdmission.setBedAvailability(null);
						trackingwithPartialAdmission.setCurrentPartialAdmission(null);
						
						if (trackingwithPartialAdmission.getCurrentStatusIsNotNull() && TrackingStatus.DECISION_TO_ADMIT.equals(trackingwithPartialAdmission.getCurrentStatus().getStatus()))
							trackingwithPartialAdmission.setCurrentStatus(createTrackingStatus(trackingwithPartialAdmission.getCurrentStatus().getPrevStatus()));
						
						String[] errors = trackingwithPartialAdmission.validate();
						if(errors != null && errors.length > 0)
						{
							engine.showErrors(errors);
							return;
						...
						try
						{
							form.getLocalContext().setPartialAdmissionForTrackingVo(domain.cancelCurrentPartialAdmission(trackingwithPartialAdmission, partialAdmission));
						}
						catch (StaleObjectException e)
						{
							engine.showMessage(form.getLocalContext().getSTALE_OBJECT_MESSAGE());
				...
				engine.open(form.getForms().Emergency.TransferDialog, new Object[] { form.getLocalContext().getPartialAdmissionForTrackingVo() != null ? form.getLocalContext().getPartialAdmissionForTrackingVo().getCurrentPartialAdmission() : null});
			} 
		...
		if (form.getLocalContext().getMessageBoxIdOnMove() != null && form.getLocalContext().getMessageBoxIdOnMove().equals(messageBoxId))
		{
			if(DialogResult.YES.equals(result))
			{
				movePatient();
		...
		if(form.getLocalContext().getDTAPlusDischargeMessageBox() != null && messageBoxId == form.getLocalContext().getDTAPlusDischargeMessageBox() )//WDEV-23019
		{
			if(DialogResult.YES.equals(result))
			{
				if(updateDNWStatus(1, false))
		...
		form.getLocalContext().setPartialAdmissionForTrackingVo(null);
		form.getLocalContext().setMessageBoxIdOnMove(null);
		form.getLocalContext().setDTAPlusDischargeMessageBox(null);
		updateControlsState();
	}
	...
	{
		LocationRefVo selectedLocation = null;
		
		if (form.grdTrackingCount().getValue() instanceof TrackingAreaForTrackingVo)
		{
			TrackingAreaForTrackingVo trackingArea = (TrackingAreaForTrackingVo) form.grdTrackingCount().getValue();
			selectedLocation = trackingArea.getEDLocation();
		}
		
		LocationRefVo selectedHospital = domain.getHospitalForSelectedLocation(selectedLocation);
		
		if (selectedHospital == null)
		{
			engine.showMessage("Could not find hospital for selected location.");
			return;
		}
		
		
		if (form.getLocalContext().getBED_MANAGEMENT_DASHBOARD_URL() == null || form.getLocalContext().getBED_MANAGEMENT_DASHBOARD_URL().length() == 0)
		{
		...
		params.add(new UrlParam("HID", selectedHospital.getID_Location().toString()));
		engine.openDashboardCloseableOnContextChange(form.getLocalContext().getBED_MANAGEMENT_DASHBOARD_URL(), params);
		
		/*
		String url = form.getLocalContext().getBED_MANAGEMENT_DASHBOARD_URL() + "?HID=" + selectedHospital.getID_Location();
	...
	//WDEV-23635
	@Override
	protected void onBtnEdDashboardClick() throws PresentationLogicException 
	{
		engine.openDashboard(ConfigFlag.GEN.ED_DASHBOARD_URL.getValue(), null);
	}
}
