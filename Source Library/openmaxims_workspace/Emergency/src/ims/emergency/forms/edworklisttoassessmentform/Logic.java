...
import ims.emergency.forms.edworklisttoassessmentform.GenForm.lyrWorklistLayer.tabClinicianWorklistContainer.GroupClinicianSortEnumeration;
import ims.emergency.forms.edworklisttoassessmentform.GenForm.lyrWorklistLayer.tabClinicianWorklistContainer.GroupClinicianWorklistFilterEnumeration;
import ims.emergency.forms.edworklisttoassessmentform.GenForm.lyrWorklistLayer.tabTriageWorklistContainer.GroupClinicianReviewEnumeration;
import ims.emergency.forms.edworklisttoassessmentform.GenForm.lyrWorklistLayer.tabTriageWorklistContainer.GroupTriageSortEnumeration;
...
import ims.emergency.vo.TriageWorklistSearchCriteriaVo;
...
public class Logic extends BaseLogic
{
	...
	private static final String	COLUMN_SEENBYNURSECOMPLASSESSMENT	= "13";     //WDEV-20429
	
	private static final int EMERGENCY_PATIENT_SUMMARY = 320; 
	private static final String CARE_CONTEXT_SEED = "CareContext_id";
	
	
	private static final int TRIAGE_LIST_ALL					= 1;
	private static final int TRIAGE_LIST_MYALLOCATED			= 2;
	private static final int TRIAGE_SORT_BY_PRIORITY			= 1;
	private static final int TRIAGE_SORT_BY_LOS					= 2;
	
	private static final int CLINICIAN_LIST_ALL					= 1;
	private static final int CLINICIAN_LIST_MYALLOCATED			= 2;
	private static final int CLINICIAN_SORT_BY_PRIORITY			= 1;
	private static final int CLINICIAN_SORT_BY_LOS				= 2;
	...
	{ 
		SEENBYCOMPLASSESSMNENT,SEENBYNURSECOMPLASSESSMNENT,EDITASSESSMENT,PATIENTSUMMARY,VIEWHISTORY,DECISIONTOAMIT,EXTERNTRANSFER //WDEV-20429
	}
	...
	private void initialize()
	{
		...
		form.lyrWorklist().tabClinicianWorklist().GroupClinicianWorklistFilter().setValue(GroupClinicianWorklistFilterEnumeration.rdoClinicianAll);
		...
		{
			initialiseAreasTriageCombo();
		}
		form.lyrWorklist().tabTriageWorklist().GroupTriageSort().setValue(GroupTriageSortEnumeration.rdoPriorityTriage);					//wdev-19374
		form.lyrWorklist().tabClinicianWorklist().GroupClinicianSort().setValue(GroupClinicianSortEnumeration.rdoPriorityClinician);		//wdev-19374
	...
	private void clearcontrols()
	{
		...
		form.lyrWorklist().tabTriageWorklist().lblTotalTriageValue().setValue("0");			//wdev-19374
		form.lyrWorklist().tabClinicianWorklist().dyngrdClinicianWorklist().getRows().clear();
		form.lyrWorklist().tabClinicianWorklist().lblTotalClinicianValue().setValue("0");	//wdev-19374
	}
	private void open()
	{
		//WDEV-18001
		...
		if( engine.getFormName().equals(form.getForms().Emergency.EDTriageWorklistToAssessmentForm))
		{
			...
			//form.lyrWorklist().tabTriageWorklist().GroupClinicianReview().setVisible(true); //WDEV-19707 
			
			if(form.getGlobalContext().Emergency.getTriageWorklistSearchCriteriaIsNotNull())
				setTriageSearchCriteria(form.getGlobalContext().Emergency.getTriageWorklistSearchCriteria());
			...
			patientsWaiting = domain.listPatientWaitings(TrackingStatus.WAITING_TO_BE_TRIAGED, engine.getCurrentLocation(),hcp, form.lyrWorklist().tabTriageWorklist().cmbAreasForTriage().getValue(), form.lyrWorklist().tabTriageWorklist().cmbUrgency().getValue());
			...
			getTriageSearchCriteria();//WDEV-19389 			
		}
		else if( engine.getFormName().equals(form.getForms().Emergency.EDClinicianWorklistToAssessmentForm))
		{
			...
			initialiseAreasClinicianCombo();				//wdev-17326
			...
			if( form.getGlobalContext().Emergency.getClinicianWorklistSearchCriteriaIsNotNull() )
			{
				...
				setClinicianSearchType(form.getGlobalContext().Emergency.getClinicianWorklistSearchCriteria().getView());//WDEV-19389 
				setClinicianSortType(form.getGlobalContext().Emergency.getClinicianWorklistSearchCriteria().getSortBy());//WDEV-19389 
			}
			...
			if( form.lyrWorklist().tabClinicianWorklist().cmbAreas().getValue() != null) //wdev-17326
			{
				...
				patientsWaiting = domain.listTrackingPatients(hcp, form.lyrWorklist().tabClinicianWorklist().cmbAreas().getValue(),form.lyrWorklist().tabClinicianWorklist().cmbTrackingStatus().getValue(), null);  //wdev-17326
	...
	private TriageWorklistSearchCriteriaVo getTriageSearchCriteria()
	{
		TriageWorklistSearchCriteriaVo searchCriteria = new TriageWorklistSearchCriteriaVo();
		
		searchCriteria.setView(getTriageSearchType());
		searchCriteria.setSortBy(getTriageSortType());
		searchCriteria.setTrackingArea(form.lyrWorklist().tabTriageWorklist().cmbAreasForTriage().getValue());
		searchCriteria.setUrgency(form.lyrWorklist().tabTriageWorklist().cmbUrgency().getValue());;
		
		return searchCriteria;
	}
	
	private void setTriageSearchCriteria(TriageWorklistSearchCriteriaVo triageWorklistSearchCriteriaVo) 
	{	
		setTriageSearchType(triageWorklistSearchCriteriaVo.getView());
		setTriageSortType(triageWorklistSearchCriteriaVo.getSortBy());
		form.lyrWorklist().tabTriageWorklist().cmbAreasForTriage().setValue(triageWorklistSearchCriteriaVo.getTrackingArea());
		form.lyrWorklist().tabTriageWorklist().cmbUrgency().setValue(triageWorklistSearchCriteriaVo.getUrgency());
	}
	
	private Integer getTriageSearchType() 
	{
		GroupClinicianReviewEnumeration searchType = form.lyrWorklist().tabTriageWorklist().GroupClinicianReview().getValue();
		if (GroupClinicianReviewEnumeration.rdoAll.equals(searchType))
		{
			return TRIAGE_LIST_ALL;
		}
		if (GroupClinicianReviewEnumeration.rdoMyAllocated.equals(searchType))
		{
			return TRIAGE_LIST_MYALLOCATED;
		}
		return null;
	}
	
	private void setTriageSearchType(Integer searchType) 
	{
		if (searchType == null)
			return;
		...
		switch (searchType)
		{
		case TRIAGE_LIST_ALL:
			form.lyrWorklist().tabTriageWorklist().GroupClinicianReview().setValue(GroupClinicianReviewEnumeration.rdoAll);
			break;
		case TRIAGE_LIST_MYALLOCATED:
			form.lyrWorklist().tabTriageWorklist().GroupClinicianReview().setValue(GroupClinicianReviewEnumeration.rdoMyAllocated);
			break;
		}		
	}
	
	private void setTriageSortType(Integer searchType) 
	{
		if (searchType == null)
			return;
		...
		switch (searchType)
		{
		case TRIAGE_SORT_BY_PRIORITY:
			form.lyrWorklist().tabTriageWorklist().GroupTriageSort().setValue(GroupTriageSortEnumeration.rdoPriorityTriage);
			break;
		case TRIAGE_LIST_MYALLOCATED:
			form.lyrWorklist().tabTriageWorklist().GroupTriageSort().setValue(GroupTriageSortEnumeration.rdoLosTriage);
			break;
		}		
	}

	private Integer getTriageSortType() 
	{
		GroupTriageSortEnumeration searchType = form.lyrWorklist().tabTriageWorklist().GroupTriageSort().getValue();
		if (GroupTriageSortEnumeration.rdoPriorityTriage.equals(searchType))
		{
			return TRIAGE_SORT_BY_PRIORITY;
		}
		if (GroupTriageSortEnumeration.rdoLosTriage.equals(searchType))
		{
			return TRIAGE_SORT_BY_LOS;
		}
		return null;
	}

	private Integer getClinicianSearchType() 
	{
		GroupClinicianWorklistFilterEnumeration searchType = form.lyrWorklist().tabClinicianWorklist().GroupClinicianWorklistFilter().getValue();
		if (GroupClinicianWorklistFilterEnumeration.rdoClinicianAll.equals(searchType))
		{
			return CLINICIAN_LIST_ALL;
		}
		if (GroupClinicianWorklistFilterEnumeration.rdoClinicianMyAllocated.equals(searchType))
		{
			return CLINICIAN_LIST_MYALLOCATED;
		}
		return null;
	}
	
	private void setClinicianSearchType(Integer searchType) 
	{
		if (searchType == null)
			return;
		...
		switch (searchType)
		{
		case CLINICIAN_LIST_ALL:
			form.lyrWorklist().tabClinicianWorklist().GroupClinicianWorklistFilter().setValue(GroupClinicianWorklistFilterEnumeration.rdoClinicianAll);
			break;
		case CLINICIAN_LIST_MYALLOCATED:
			form.lyrWorklist().tabClinicianWorklist().GroupClinicianWorklistFilter().setValue(GroupClinicianWorklistFilterEnumeration.rdoClinicianMyAllocated);
			break;
		}		
	}
	
	private void setClinicianSortType(Integer searchType) 
	{
		if (searchType == null)
			return;
		...
		switch (searchType)
		{
		case CLINICIAN_SORT_BY_LOS:
			form.lyrWorklist().tabClinicianWorklist().GroupClinicianSort().setValue(GroupClinicianSortEnumeration.rdoLosClinician);
			break;
		case CLINICIAN_SORT_BY_PRIORITY:
			form.lyrWorklist().tabClinicianWorklist().GroupClinicianSort().setValue(GroupClinicianSortEnumeration.rdoPriorityClinician);
			break;
		}		
	}

	private Integer getClinicianSortType() 
	{
		GroupClinicianSortEnumeration searchType = form.lyrWorklist().tabClinicianWorklist().GroupClinicianSort().getValue();
		if (GroupClinicianSortEnumeration.rdoPriorityClinician.equals(searchType))
		{
			return CLINICIAN_SORT_BY_PRIORITY;
		}
		if (GroupClinicianSortEnumeration.rdoLosClinician.equals(searchType))
		{
			return CLINICIAN_SORT_BY_LOS;
		}
		return null;
	}
	...
	private void createdDdyngrdClinicianWorklist()
	{
		...
		columnName = form.lyrWorklist().tabClinicianWorklist().dyngrdClinicianWorklist().getColumns().newColumn(" ", COLUMN_SEENBYNURSECOMPLASSESSMENT);  //WDEV-20429
		columnName.setWidth(120);
		columnName.setReadOnly(false);
	...
	private void addRowToTriageDyngrd(TrackingForDisplayClinicianAndTriageWorklistVo tempVo)//WDEV-18001
	{
		...
		row.setSelectable(false);//WDEV-19623
		...
		if( engine.hasRight(AppRight.ED_CAN_PERFORM_ADMISSION))
		{
			...
			boolean isValidPatIDForAdmission = PatIdType.NHSN.equals(dispIdType)? (PatIdType.NHSN.equals(primaryIDFromPASroviderSystem) ? false : tempVo != null && tempVo.getPatientIsNotNull() && primaryIDFromPASroviderSystem!=null && tempVo.getPatient().getPatId(primaryIDFromPASroviderSystem) != null) 
										: (tempVo != null && tempVo.getPatientIsNotNull() && tempVo.getPatient().getPatId(dispIdType) != null);
			
			if (isValidPatIDForAdmission) //wdev-17110 //WDEV-21171 //WDEV-23527
			{
				//wdev-17138
				...
				cellOptions = new TableCellOptions();
								 ...
								 cellOptions.setButtonImage(form.getImages().Emergency.DTA16);		//wdev-17329
									...
									if (ConfigFlag.GEN.ED_DTA_ADMISSION_FUNCTIONALITY.getValue().equals("PENDING"))
									{
										cellOptions.setButtonImageText("  Bed Request");
									}
									else
	...
	private void addRowToClinicianDyngrd(TrackingForDisplayClinicianAndTriageWorklistVo tempVo)//WDEV-17131 //WDEV-18001
	{
		...
		TableCellOptions cellOptions = new TableCellOptions();
						 ...
						 cellOptions.setButtonImageText("Seen By Medic"); //WDEV-20429
		...
		//WDEV-20429
		cellLabel = row.getCells().newCell(getColumnClin(COLUMN_SEENBYNURSECOMPLASSESSMENT), DynamicCellType.TABLE);
		helper = new DynamicGridCellTable(cellLabel);		
		tableRow = helper.getRows().newRow();
		tableCell = tableRow.getCells().newCell();
		  		  tableCell.setType(TableCellType.BUTTON);		
		...
		cellOptions = new TableCellOptions();
						 cellOptions.setAutopostback(true);
						 cellOptions.setButtonImage(form.getImages().Core.FirstAidKit24);		//wdev-17329
						 cellOptions.setButtonImageText("Seen By Nurse");
						 cellOptions.setButtonHeight(111);
						 cellOptions.setButtonWidth(110);
						 cellOptions.setAlign(Align.CENTER);
	
		tableCell.setOptions(cellOptions);
		tableCell.setIdentifier(SelectedClinicianWorklistButtons.SEENBYNURSECOMPLASSESSMNENT);
		cellLabel.setValue(helper);
		//----------
	...
	private Integer addNameClinicianRow(TrackingForDisplayClinicianAndTriageWorklistVo tracking, DynamicGridRow row)//WDEV-17131 //WDEV-18001
	{
		...
		if (tracking.getPatientIsNotNull() )	//wdev-19169
		{
			//wdev-19169
			if( ims.core.vo.lookups.PatIdType.getNegativeInstance(ims.configuration.ConfigFlag.UI.DISPLAY_PATID_TYPE.getValue()).equals(ims.core.vo.lookups.PatIdType.MRNNUM) && tracking.getPatient().getPatId(ims.core.vo.lookups.PatIdType.MRNNUM) == null )
			{
				tableCell.setValue(tracking.getPatient().getMRNStatusIsNotNull() ? tracking.getPatient().getMRNStatus().getIItemText():null);
				tableCell.setTooltip(tracking.getPatient().getMRNStatusIsNotNull() ? tracking.getPatient().getMRNStatus().getIItemText():null);
			}
			else
			{
				if( tracking.getPatient().getPatId(ims.core.vo.lookups.PatIdType.getNegativeInstance(ims.configuration.ConfigFlag.UI.DISPLAY_PATID_TYPE.getValue())) != null )
				{
					tableCell.setValue(tracking.getPatient().getPatId(ims.core.vo.lookups.PatIdType.getNegativeInstance(ims.configuration.ConfigFlag.UI.DISPLAY_PATID_TYPE.getValue())).getValue());
					tableCell.setTooltip(tracking.getPatient().getPatId(ims.core.vo.lookups.PatIdType.getNegativeInstance(ims.configuration.ConfigFlag.UI.DISPLAY_PATID_TYPE.getValue())).getValue());
				}
	...
	private Integer addNameTriageRow(TrackingForDisplayClinicianAndTriageWorklistVo tracking, DynamicGridRow row)//WDEV-18001
	{
		...
		if(tracking.getEpisode() != null && tracking.getEpisode().getPresentingComplaint() != null)
		{
			tableCell.setValue(tracking.getEpisode().getPresentingComplaint().getText());
		}
		tableCell.setWidth(140);
		tableCell.setColSpan(3);
	...
	{
		form.getGlobalContext().Emergency.setTriageWorklistSearchCriteria(getTriageSearchCriteria());//WDEV-19389 
		open();	
	}
	...
	class TriageLOSComparator implements Comparator<TrackingForDisplayClinicianAndTriageWorklistVo> //WDEV-18001
	{
		...
		public int compare(TrackingForDisplayClinicianAndTriageWorklistVo o1, TrackingForDisplayClinicianAndTriageWorklistVo o2) //WDEV-18001,wdev-19374
		{
			...
			{
				if (o1 != null && o1.getAttendance() != null && o1.getAttendance().getArrivalDateTime() != null && o2!=null && o2.getAttendance() != null && o2.getAttendance().getArrivalDateTime() != null)
				{
					Integer LOS1 = calculateDateDiffInMinutes(new DateTime(), o1.getAttendance().getArrivalDateTime());
					Integer LOS2 = calculateDateDiffInMinutes(new DateTime(), o2.getAttendance().getArrivalDateTime());
					
					return order*LOS1.compareTo(LOS2);
				}
				if (o1 == null || o1.getAttendance() == null || o1.getAttendance().getArrivalDateTime() == null)
					return -1*order;
				if (o2 == null || o2.getAttendance() == null || o2.getAttendance().getArrivalDateTime() == null)
					return order;
	...
	//wdev-19374
	class TriagePriorityComparator implements Comparator<TrackingForDisplayClinicianAndTriageWorklistVo> //WDEV-18001
	{
		private int order =1;
		public TriagePriorityComparator()
		{
			order = 1;
		}
		public TriagePriorityComparator(SortOrder order)
		{
			this.order = SortOrder.ASCENDING.equals(order)? 1:-1;
		}
	...
	class ClinicianPriorityComparator implements Comparator<TrackingForDisplayClinicianAndTriageWorklistVo> //WDEV-18001
	{
		private int order =1;
		public ClinicianPriorityComparator()
		{
			order = 1;
		}
		public ClinicianPriorityComparator(SortOrder order)
		{
			this.order = SortOrder.ASCENDING.equals(order)? 1:(-1);
		}
		public int compare(TrackingForDisplayClinicianAndTriageWorklistVo o1, TrackingForDisplayClinicianAndTriageWorklistVo o2) //WDEV-18001
	...
	class ClinicianLOSComparator implements Comparator<TrackingForDisplayClinicianAndTriageWorklistVo> //WDEV-18001
	{
		private int order =1;
		public ClinicianLOSComparator()
		{
			order = 1;
		}
		public ClinicianLOSComparator(SortOrder order)
		{
			this.order = SortOrder.DESCENDING.equals(order)?-1:1;
		}
		public int compare(TrackingForDisplayClinicianAndTriageWorklistVo o1, TrackingForDisplayClinicianAndTriageWorklistVo o2) //WDEV-18001
	...
	@Override
	protected void onCmbAreasForTriageValueChanged() throws PresentationLogicException
	{
		form.getGlobalContext().Emergency.setTriageWorklistSearchCriteria(getTriageSearchCriteria());//WDEV-19389 
		open();
	}
	...
	@Override
	protected void onRadioButtonGroupClinicianWorklistFilterValueChanged() throws PresentationLogicException
	{
		getClinicianSearchCriteria();
		open();
	}

	private void getClinicianSearchCriteria()
	{
		...
		tempVo.setView(getClinicianSearchType());
		tempVo.setSortBy(getClinicianSortType());
		form.getGlobalContext().Emergency.setClinicianWorklistSearchCriteria(tempVo);
		
	}

	//wdev-19374
	protected void onRadioButtonGroupTriageSortValueChanged() throws PresentationLogicException 
	{
		form.getGlobalContext().Emergency.setTriageWorklistSearchCriteria(getTriageSearchCriteria());//WDEV-19389 
		open();
	}
	...
	//wdev-19374
	protected void onRadioButtonGroupClinicianSortValueChanged() throws PresentationLogicException 
	{
		getClinicianSearchCriteria();
		open();	
	}
