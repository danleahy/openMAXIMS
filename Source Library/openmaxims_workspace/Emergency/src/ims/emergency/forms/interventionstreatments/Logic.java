...
public class Logic extends BaseLogic
{
	...
	private boolean save() 
	{
		...
		String[] errors = record.validate(validateUIControls()); //WDEV-19406
		if(errors != null && errors.length > 0)
		{
			engine.showErrors("Validation Errors", errors);//WDEV-19406
			return false;
		}
		
		try 
		{
			form.getGlobalContext().Emergency.setInterventionsTreatmentsForAttendence(domain.save(record, form.getLocalContext().getAddedDuringCoding()));
		} 
	...
	private String[] validateUIControls()
	{
		...
		int emptyRowsNo = 0;
		if (form.grdSelected().getRows() == null || form.grdSelected().getRows().size() == 0)
			errors.add("At least one treatment/intervention is mandatory for selection");
		else
		{	
			for (int i=0; i<form.grdSelected().getRows().size(); i++)
			{
				...
				if (form.grdSelected().getRows().get(i).getValue() instanceof TreatmentInterventionType && form.grdSelected().getRows().get(i).getRows().size() == 0)
					emptyRowsNo++;

			}
			if  (emptyRowsNo > 0 && emptyRowsNo == form.grdSelected().getRows().size())
				errors.add("At least one treatment/intervention is mandatory for selection");
		}	
		if (errors.size() != 0) // validate UI for Authoring Info, else they are validated in the VO	
		{	
			if (Boolean.TRUE.equals(form.getLocalContext().getAuthoringInfoControlIsEnabled()) && form.ccAuthoringInfo().getErrors() != null)
				errors.add(form.ccAuthoringInfo().getErrors());
		}
