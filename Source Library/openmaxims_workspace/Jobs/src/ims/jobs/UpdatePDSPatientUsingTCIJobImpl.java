...
public final class UpdatePDSPatientUsingTCIJobImpl extends ims.scheduler.SchedulerJob
{
	...
	public SchedulerJobExecutionSummary doExecute() throws JobExecutionException
	{
		...
		if( configVo == null || !configVo.getNumOfDaysTCIBatchTraceIsNotNull())
		{
		...
		else
		{
    		...
    		PatientForJobVoCollection patients = listPatientsFromFromTCIPatientElectiveList(job, configVo);		//get Patient from Booking appointments
	...
	private PatientForJobVoCollection listPatientsFromFromTCIPatientElectiveList(PDSApptLetterRunVo job, PDSConfigurationVo configVo)
	{
		...
		if( configVo != null && configVo.getNumOfDaysTCIBatchTrace() != null )
			todayMinus = new DateTime( todayd.addDay((configVo.getNumOfDaysTCIBatchTrace())*(-1))).getJavaDate();
			
		DomainFactoryBridge factory = getDomainFactory();
				
		String hsql = "select p2_1 from PatientElectiveList as p1_1 left join p1_1.patient as p2_1 left join p1_1.tCIDetails as t1_1 where (t1_1.tCIDate >= :fromDate and t1_1.tCIDate <= :toDate and p2_1.pDSDateLastChecked < :todayminus)";
	...
	private boolean checkAndRetrievalPdsDemographics(PatientShort pat, InititPatAndPDSPATForJobVo patTempVo, MissingMappingVoCollection missingMappings) 
    {
        ...
        if( pat.getNhsn() != null)
        {
			...
			{
				System.out.println("UpdatePDSPatientUsingTCIJobImpl: " + e.toString());
				return false;
			}
		}
        else
