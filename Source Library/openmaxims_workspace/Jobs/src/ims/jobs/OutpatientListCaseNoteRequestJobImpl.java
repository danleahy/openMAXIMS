...
import ims.core.vo.OutpatientRequestsBatchVo;
...
public final class OutpatientListCaseNoteRequestJobImpl extends ims.scheduler.SchedulerJob
{
	...
	public SchedulerJobExecutionSummary doExecute() throws JobExecutionException
	{
		...
		OutpatientRequestsBatchVo job = getJobDetails();
		if (job == null)
		{
		...
		NDays = job.getPrintRequestsForOutpatientAppointmentsInNdays();
		
		if(NDays == null)
			throw new JobExecutionException("Print Requests for outpatient appointments in N days is not set.");
		
		CaseNoteRequestJobVoCollection requests = getRequests();
		
		try
		{
			saveRequestsAndPrint(requests, job);
		}
		catch(Exception e)
		{
	...
	private synchronized void saveRequestsAndPrint(CaseNoteRequestJobVoCollection requests, OutpatientRequestsBatchVo job) throws Exception
	{
		if(requests == null)
			return;
		...
		Object[] obj = getSystemReportAndTemplate(OUTPATIENT_CASE_NOTE_REQUESTS_LIST_REPORT_ID);
		...
		for(CaseNoteRequestJobVo request : requests)
		{
			...
			client.addSeed(new SeedValue("PatientCaseNoteRequest_id",  request.getID_PatientCaseNoteRequest(), Integer.class));
		}
	...
	private CaseNoteRequestJobVoCollection getRequests() throws JobExecutionException
	{
		DomainFactoryBridge factory = getDomainFactory();
		...
		String query = "select req from PatientCaseNoteRequest as req left join req.caseNote as cn left join cn.currentLocation as rLoc left join rLoc.type as locType left join req.requestStatus as rStatus where req.appointment is not null and locType.id = :Type and req.requiredByDate between :DateFrom and :DateTo and req.printedPullListDate is null and rStatus.id = :StatusId";
	...
	private synchronized void saveJob(OutpatientRequestsBatchVo job)
	{
		if(job == null)
			return;
		...
		DomainFactoryBridge factory = getDomainFactory();
		OutpatientRequestsBatch doJob = OutpatientRequestsBatchVoAssembler.extractOutpatientRequestsBatch(factory, job);
		try 
		{
			factory.save(doJob);
		} 
	...
	private OutpatientRequestsBatchVo getJobDetails()
	{
		OutpatientRequestsBatch doJob = OutpatientRequestsBatch.getOutpatientRequestsBatchFromConfiguredJobDetails(getDomainFactory(), super.getConfiguredJob().getConfiguredScheduledJobID());
		return OutpatientRequestsBatchVoAssembler.create(doJob);
	}
