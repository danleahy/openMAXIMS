...
public class Logic extends BaseLogic
{
	...
	private static final int MAX_LENGHT = 105;
	...
	protected void onFormOpen(Object[] args) throws PresentationLogicException 
	{
		...
		form.lblOriginOfScore().setValue(formatScoreDetails(form.getGlobalContext().Core.getVitalSignsEscalationText()));
		
		//WDEV-20311
		form.getLocalContext().setSECSVo(domain.getSECSConfig());
		form.lblScore().setValue(form.getLocalContext().getSECSVo()!=null && form.getLocalContext().getSECSVo().getOBSProtocolTypeIsNotNull() ? form.getLocalContext().getSECSVo().getOBSProtocolType().getText() + " Score:": "OBS Score:");
	...
	protected void onBtnSaveClick() throws PresentationLogicException {
		...
		patientSecs.setScoreDetails(form.getGlobalContext().Core.getVitalSignsEscalationText());
		patientSecs.setActionTaken(form.txtActionTaken().getValue());
		
		//WDEV-20311
		patientSecs.setOBSProtocolType(form.getLocalContext().getSECSVo()!=null ? form.getLocalContext().getSECSVo().getOBSProtocolType() : null);
	...
	private String formatScoreDetails(String original)
	{
		...
		int lastEnter = value.lastIndexOf("\n");
		
		if(lastEnter == -1)
			return original;
		
		a.append(value.substring(0, lastEnter));
		
		String urine = value.substring(lastEnter, value.length());
	...
	private String wrap(String s) 
	{
		...
		int i = 0;
		while (i + MAX_LENGHT < sb.length() && (i = sb.lastIndexOf(" ", i + MAX_LENGHT)) != -1) 
		{
		    sb.replace(i, i + 1, "\n");
		}
