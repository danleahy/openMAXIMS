...
import ims.RefMan.vo.ClinicalOutcomeSearchCriteriaVo;
...
public class Logic extends BaseLogic
{
	...
	private static final int LIST_ALL_REFERRALS			= 1;
	private static final int LIST_THIS_REFERRAL			= 2;
	// ------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	protected void onFormOpen(Object[] args) throws PresentationLogicException
	{
		...
		// Refresh screen		
		//WDEV-19389 - start
		if (!(form.getGlobalContext().RefMan.getCatsReferralIsNotNull() && form.getGlobalContext().RefMan.getClinicalOutcomeSearchCriteriaIsNotNull() && form.getGlobalContext().RefMan.getClinicalOutcomeSearchCriteria().getCatsReferralIsNotNull() && form.getGlobalContext().RefMan.getCatsReferral().equals(form.getGlobalContext().RefMan.getClinicalOutcomeSearchCriteria().getCatsReferral())))
			form.getGlobalContext().RefMan.setClinicalOutcomeSearchCriteria(null);
		
		if (form.getGlobalContext().RefMan.getClinicalOutcomeSearchCriteriaIsNotNull())
		{
			setSearchCriteria(form.getGlobalContext().RefMan.getClinicalOutcomeSearchCriteria());
		}
		//WDEV-19389 - end
		open();
	}

	private ClinicalOutcomeSearchCriteriaVo getSearchCriteria()
	{
		ClinicalOutcomeSearchCriteriaVo searchCriteria = new ClinicalOutcomeSearchCriteriaVo();

		searchCriteria.setSearchType(getSearchType());
		searchCriteria.setProcedure(form.cmbIntraOperativeDetails().getValue());
		searchCriteria.setCatsReferral(form.getGlobalContext().RefMan.getCatsReferral());

		return searchCriteria;
	}

	private void setSearchCriteria(ClinicalOutcomeSearchCriteriaVo clinicalOutcomeSearchCriteriaVo) 
	{
		setSearchType(clinicalOutcomeSearchCriteriaVo.getSearchType());
		try
		{
		...
		{e.printStackTrace();}
		form.cmbIntraOperativeDetails().setValue(clinicalOutcomeSearchCriteriaVo.getProcedure());
		try
		{
	...
	private void setSearchType(Integer searchType)
	{
		...
		case LIST_ALL_REFERRALS:
			form.GroupView().setValue(GroupViewEnumeration.rdoAllRefferals);
			break;
		case LIST_THIS_REFERRAL:
	...
	{
		GroupViewEnumeration searchType = form.GroupView().getValue();
		if (GroupViewEnumeration.rdoAllRefferals.equals(searchType))
		{
			return LIST_ALL_REFERRALS;
		}
		if (GroupViewEnumeration.rdoThisReferral.equals(searchType))
		{
			return LIST_THIS_REFERRAL;
		}
	...
	protected void onRadioButtonGroupViewValueChanged() throws PresentationLogicException
	{
		...
		form.getGlobalContext().RefMan.setClinicalOutcomeSearchCriteria(getSearchCriteria());//WDEV-19389 
