...
public class Logic extends BaseLogic
{
	...
	public void clearInstanceControls()
	{
		...
		form.lyrContent().tabDischargeDetails().dtimDischarge().setValue(null);
	...
	public String[] validateUIRules(boolean discharge)
	{
		ArrayList<String> listOfErrors = new ArrayList<String>();
		
		Boolean wasDischarged = form.getLocalContext().getDischargeSummaryPacuAndWard() != null && domain.getDischargedEpisode(form.getLocalContext().getDischargeSummaryPacuAndWard().getDischargeEpisode()) != null;
		
		if (form.lyrContent().tabDischargeDetails().dtimDischarge().getValue() == null && (discharge || wasDischarged))
		{
			listOfErrors.add("Discharge Date/Time is mandatory.");
		}
		
		if (form.lyrContent().tabDischargeDetails().dtimDischarge().getValue() != null && form.lyrContent().tabDischargeDetails().dtimDischarge().getValue().isGreaterThan(new DateTime()))
		{
			listOfErrors.add("Discharge Date/Time cannot be in the future.");
		}
		
		DateTime dateTimeFromGlobalContext = domain.getAdmissionDateTime(form.getGlobalContext().RefMan.getCatsReferral());
		...
		{
			DateTime dateTimeFromLocalContext = domain.getCurrentAddmissionDateTime(form.getLocalContext().getDischargeSummaryPacuAndWard().getDischargeEpisode());
			
			// Check discharge date against previously used admission date, not the current one
			if (form.lyrContent().tabDischargeDetails().dtimDischarge().getValue() != null
				&& dateTimeFromLocalContext != null) // wdev-19922
			{
				if (form.lyrContent().tabDischargeDetails().dtimDischarge().getValue().getDate().isLessThan(dateTimeFromLocalContext.getDate()) ||
					(form.lyrContent().tabDischargeDetails().dtimDischarge().getValue().getDate().equals(dateTimeFromLocalContext.getDate())
					   && form.lyrContent().tabDischargeDetails().dtimDischarge().getValue().getTime().isLessThan(dateTimeFromLocalContext.getTime())))
				{
					listOfErrors.add("Discharge Date/Time cannot be earlier than Admission Date/Time. " + dateTimeFromLocalContext.toString()); //WDEV-18762
				}
			}
		}
		else if (form.lyrContent().tabDischargeDetails().dtimDischarge().getValue() != null
				&& dateTimeFromGlobalContext != null) // wdev-19922
		{
			if (form.lyrContent().tabDischargeDetails().dtimDischarge().getValue().getDate().isLessThan(dateTimeFromGlobalContext.getDate()) || 
					(form.lyrContent().tabDischargeDetails().dtimDischarge().getValue().getDate().equals(dateTimeFromGlobalContext.getDate()) 
					&& form.lyrContent().tabDischargeDetails().dtimDischarge().getValue().getTime().isLessThan(dateTimeFromGlobalContext.getTime())))
			{
				listOfErrors.add("Discharge Date/Time cannot be earlier than Admission Date/Time. " + dateTimeFromGlobalContext.toString() ); //WDEV-18762
			}
		}
		
		if (form.lyrContent().tabDischargeDetails().cmbDischargeType().getValue() == null && (discharge || wasDischarged))
		{
			listOfErrors.add("Discharge Type is mandatory.");
		}
	...
	public boolean save(boolean discharge, boolean check) throws ims.framework.exceptions.PresentationLogicException
	{
		...
		//WDEV-16967
		String[] uiErrors = validateUIRules(discharge);
		...
		String[] errors =  form.getLocalContext().getDischargeSummaryPacuAndWard().validate();
		...
		//WDEV-11899 - starts here
		if(check && form.getLocalContext().getDischargeSummaryPacuAndWard().getDischargeDateTime() != null && new Date().isGreaterThan(form.getLocalContext().getDischargeSummaryPacuAndWard().getDischargeDateTime().getDate()))
		{
		...
		try
		{
			...
			{
				voCats.setDischargeDate(form.lyrContent().tabDischargeDetails().dtimDischarge().getValue().getDate());
				...
				|| (!form.getLocalContext().getDischargeSummaryPacuAndWard().getDischargeEpisodeIsNotNull() && form.getLocalContext().getInpatientIsNotNull() && discharge))
			{
				...
				dischargeEpisode.setDischargeDateTime(form.lyrContent().tabDischargeDetails().dtimDischarge().getValue());
	...
	private void populateRecordBrowser(DischargeSummaryPacuAndWardLiteVoCollection dischargeSummaryPacuAndWardLiteVoCollection)
	{
		...
		for (int i = 0; i < dischargeSummaryPacuAndWardLiteVoCollection.size(); i++)
		{
			...
			text.append(dischargeSummary.getDischargeDateTimeIsNotNull() ? dischargeSummary.getDischargeDateTime() : "");
			text.append(" - ");
	...
	private void populateScreenFromInstance(DischargeSummaryPacuAndWardVo dischargeSummary)
	{
		...
		form.lyrContent().tabDischargeDetails().dtimDischarge().setValue(dischargeSummary.getDischargeDateTime());
	...
	private DischargeSummaryPacuAndWardVo populateInstanceFromScreen(DischargeSummaryPacuAndWardVo dischargeSummaryPacuAndWard)
	{
		...
		dischargeSummaryPacuAndWard.setDischargeDateTime(form.lyrContent().tabDischargeDetails().dtimDischarge().getValue());
	...
	private Booking_AppointmentVo cancelAppointmentFinish()
	{
		...
		do
		{
			Booking_AppointmentVo appointment = null;
			try
			{
				appointment = domain.getAppointmentFull(form.getLocalContext().getAppointmentToCancel());
			}
			...
			try
			{
				if (form.getGlobalContext().Scheduling.getApptCancelStatusIsNotNull())
				{
					...
					statusVo.setSession(appointment.getSessionIsNotNull() ? appointment.getSession() : null); //WDEV-23185
					...
					appointment.setLinkedApptsToBeCancelled(form.getGlobalContext().RefMan.getLinkedAppointmentsCollection());//WDEV-19543
					//WDEV-19445
					if (Boolean.TRUE.equals(form.getGlobalContext().Scheduling.getApptCancelStatus().getCancelTCI()) && Boolean.TRUE.equals(form.getGlobalContext().Scheduling.getApptCancelStatus().getCancelledForNonMedicalReason()) && appointment.getTheatreBookingIsNotNull() && Status_Reason.HOSPITALCANCELLED.equals(form.getGlobalContext().Scheduling.getApptCancelStatus().getStatusReason()) && form.getGlobalContext().Scheduling.getApptCancelStatus().getStatusChangeDateTimeIsNotNull() && form.getGlobalContext().Scheduling.getApptCancelStatus().getStatusChangeDateTime().getDate().equals(form.getGlobalContext().Scheduling.getApptCancelStatus().getTCIDate()))
					{
						appointment.setDate28DayRuleBreach(new DateTime(appointment.getAppointmentDate(), appointment.getApptStartTime()).addDays(28).getDate());
					}
					...
					domain.updateCatsReferralAdditionalInvStatus(form.getGlobalContext().RefMan.getCatsReferral(), appointment);
				}
				return appointment;
	...
	private void addMedication()
	{
		...
		form.getGlobalContext().RefMan.setDrugsAlreadyAddedToPrescription(getDrugsFromPrescription()); //WDEV-20283
