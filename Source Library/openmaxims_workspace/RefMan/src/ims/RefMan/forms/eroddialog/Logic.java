...
public class Logic extends BaseLogic
{
	...
	private void clear()
	{
		...
		//WDEV-19616 
		form.ctnDetails().cmbDelayReason().setValue(null);
		form.ctnDetails().txtReason().setValue(null);
		form.ctnDetails().chkExtendedDelay().setValue(null);//wdev-20431
	...
	private ReferralERODVo populateDataFromScreen(ReferralERODVo referralERODVo)
	{
		...
		//WDEV-19616 
		referralERODVo.setERODReason(form.ctnDetails().cmbDelayReason().getValue());
		referralERODVo.setERODReasonComment(form.ctnDetails().txtReason().getValue());
		...
		referralERODVo.setIsExtendedDelay(form.ctnDetails().chkExtendedDelay().getValue());
	...
	private void updateControlsState()
	{
		...
		form.ctnDetails().dteERODDate2().setRequired(!form.ctnDetails().chkExtendedDelay().getValue()); 
	...
	private void populateERODDetails(ReferralERODVo erod)
	{
		...
		form.ctnDetails().chkExtendedDelay().setValue(erod.getIsExtendedDelay());

		//WDEV-19616 
		form.ctnDetails().cmbDelayReason().setValue(erod.getERODReason());
		form.ctnDetails().txtReason().setValue(erod.getERODReasonComment());
	...
	protected void onMessageBoxClosed(int messageBoxId, DialogResult result) throws PresentationLogicException
	{
		if(form.getLocalContext().getReasonableDateMessageBoxId() != null && form.getLocalContext().getReasonableDateMessageBoxId() == messageBoxId)
		{
			...
			{
				if( ConfigFlag.DOM.EROD_SAVE_ONLY_REASONABLE_OFFERS.getValue() == false )	//wdev-19200
				{
					
					if(save())
						open();
				}
				form.ctnDetails().chkReasonableOffer().setValue(null);						//wdev-19200
			}
	...
	private void doRIE()
	{
		...
		try
		{
			// WDEV-20704 - We may need to RIE a Pause record for the current clock
			domain.inactivatePause(form.getGlobalContext().RefMan.getCatsReferral(), form.grdERODs().getValue());
