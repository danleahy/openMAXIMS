...
import ims.clinical.forms.cliniclistwithicpactions.Logic.ImageComparator;
...
public class Logic extends BaseLogic
{
	...
	private void initialise()
	{
		...
		*/
		...
		form.btnAdmit().setVisible(false);//WDEV-20377
	...
	private void fillRow(grdTheatreRow row, TheatreListBookingAppointmentVo item, TheatreTrackingIdleConfigVo theatreTrackingIdleConfig) 
	{
		...
		{
			TCIForPatElectListForWardLiteVo tempTciVo = domain.getTCIForPatElectiveList(item);
			if( tempTciVo != null && tempTciVo.getTCIWardIsNotNull())
			{
				row.setColLocation(tempTciVo.getTCIWard().getIItemText());
				row.setTooltipForColLocation(tempTciVo.getTCIWard().getIItemText());
			}
		}
	...
	private void updateContextMenus() 
	{
		...
		if (form.grdTheatre().getSelectedRow() != null && canBeAdmitted && isNotACurrentInpatient)//	WDEV-13307
		{
			//form.getContextMenus().RefMan.getTheatreListMenuADDItem().setVisible(true); //WDEV-20377
			form.btnAdmit().setEnabled(true);
		}
		
		//WDEV-9657 DNA //WDEV-18442
		if (form.grdTheatre().getSelectedRow() != null && form.grdTheatre().getValue() != null && canBeDNAed(form.grdTheatre().getValue().getApptStatus(), form.grdTheatre().getValue().getSessionIsNotNull() ? form.grdTheatre().getValue().getSession().getSessionDate() : null, form.grdTheatre().getValue().getApptStartTime())) //WDEV-11863
		...
		//WDEV-18442 
		//form.getContextMenus().RefMan.getTheatreListMenuRETURNTOTHEATREItem().setVisible(form.grdTheatre().getSelectedRow() != null && canBeAdmitted && !isNotACurrentInpatient);//WDEV-13307 //WDEV-20377
		//wdev-16163 //WDEV-18442 
		form.getContextMenus().RefMan.getTheatreListMenuATTACHICPTOBOOKINGItem().setVisible(form.grdTheatre().getSelectedRow() != null && form.grdTheatre().getValue() != null && !form.grdTheatre().getValue().getICPInfoIsNotNull() && engine.hasRight(AppRight.CAN_ATTACH_APPOINTMENT_TO_ICP) ); 
		//-----------
		//WDEV-18442
		form.getContextMenus().RefMan.getTheatreListMenuCANCEL_THEATRE_APPTItem().setVisible(form.grdTheatre().getSelectedRow() != null && form.grdTheatre().getValue() != null && Status_Reason.BOOKED.equals(form.grdTheatre().getValue().getApptStatus()) && Boolean.TRUE.equals(ConfigFlag.UI.CAN_CONFIGURE_THEATRE_PROFILES.getValue()));
	}
...
//		form.getGlobalContext().Scheduling.setBookingAppointment(domain.getBookingAppt(form.grdTheatre().getValue()));
	...
	private void loadSessions()
	{
		...
		{
			SessionShortVoCollection listSessions = domain.listSession(form.dteTheatre().getValue(), (HcpRefVo) form.ccListOwner().getValue(), form.cmbHospital().getValue(),form.cmbSessionType().getValue(),form.cmbListType().getValue());	//wdev-19419
...
//			dnaPatient(form.grdTheatre().getValue());
	...
	public class ImageComparator implements Comparator<Object>
	{
		...
		public int compare(Object ob1, Object ob2)
		{
			...
			if (ob1 instanceof TheatreListBookingAppointmentVo)
			{
				TheatreListBookingAppointmentVo o1 = (TheatreListBookingAppointmentVo) ob1;
				isICP1 = (o1.getICPInfo() != null ? 1 : 0);
			}

			if (ob2 instanceof TheatreListBookingAppointmentVo)
			{
				TheatreListBookingAppointmentVo o2 = (TheatreListBookingAppointmentVo) ob2;
				isICP2 = (o2.getICPInfo() != null ? 1 : 0);
			}

			if (isICP1 != 0 && isICP2 != 0)
			{
				return isICP1.compareTo(isICP2) * direction;
			}
	...
	@Override
	protected void onGrdTheatreGridHeaderClicked(int column) throws PresentationLogicException 
	{
		int gridRowsSize = form.grdTheatre().getRows() != null ? form.grdTheatre().getRows().size() : 0;
		if (gridRowsSize <= 1) 
			return;
		...
		if (column == 0)
		{
			...
			TheatreListBookingAppointmentVoCollection voIPColl = new TheatreListBookingAppointmentVoCollection();
			for (int i = 0 ; i < gridRowsSize; i++)
				voIPColl.add(((TheatreListBookingAppointmentVo) form.grdTheatre().getRows().get(i).getValue()));	
			
			voIPColl.sort( new ImageComparator(form.getLocalContext().getSortOrderICPImage()));
