...
import ims.ocrr.forms.patientresultsandorders.GenForm.grdClinicalImagingRow;
import ims.ocrr.forms.patientresultsandorders.GenForm.grdDisciplineRow;
import ims.ocrr.forms.patientresultsandorders.GenForm.grdPathologyRow;
import ims.ocrr.forms.patientresultsandorders.GenForm.grdPathologyRowCollection;
...
public class Logic extends BaseLogic
{
	...
	{
		if (form.getForms().OCRR.SearchDisciplines.equals(formName) && DialogResult.OK.equals(result))
		{
			// Populate search disciplines in Disciplines grid
			populateDisciplines(form.getGlobalContext().OCRR.getSelectedDisciplines());
		}
		else
		{
			search();
	...
	@Override
	protected void onImbFindClick() throws PresentationLogicException
	{
		search();		
	}
	...
	@Override
	protected void onBtnChooseDisciplineClick() throws PresentationLogicException
	{
		form.getGlobalContext().OCRR.setSelectedDisciplines(form.grdDiscipline().getValues());
	...
	{
		form.dteFrom().setValue(new Date().addDay(-28));
	...
	private void populateDisciplines(ServiceLiteVoCollection selectedDisciplines)
	{
		form.grdDiscipline().getRows().clear();
		...
		for (ServiceLiteVo service : selectedDisciplines)
		{
			grdDisciplineRow disciplineRow = form.grdDiscipline().getRows().newRow();
			
			disciplineRow.setcolDiscipline(service.getServiceName());
			disciplineRow.setValue(service);
		}
	}
}
