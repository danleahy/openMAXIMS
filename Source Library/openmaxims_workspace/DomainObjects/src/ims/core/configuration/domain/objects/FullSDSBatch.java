...
public class FullSDSBatch extends ims.domain.DomainObject implements ims.domain.SystemInformationRetainer, java.io.Serializable {
	public static final int CLASSID = 1028100093;
	private static final long serialVersionUID = 1028100093L;
    ...
    public FullSDSBatch (Integer id, int ver)
    {
    	super(id, ver);
    }
    public FullSDSBatch ()
    ...
    public FullSDSBatch (Integer id, int ver, Boolean includeRecord)
    {
    	super(id, ver, includeRecord);
    }
	public Class getRealDomainClass()
	{
		return ims.core.configuration.domain.objects.FullSDSBatch.class;
	}
	...
	public static FullSDSBatch getFullSDSBatchFromConfiguredJobDetails(ims.domain.ILightweightDomainFactory factory, Integer id)
	{
		java.util.List l = factory.find("from FullSDSBatch c where c.configuredJobDetails.id = " + id);
		if (l == null || l.size() == 0) return null;
		if (l.size() > 1) throw new ims.domain.exceptions.DomainRuntimeException("Non-unique get call. FullSDSBatch.configuredJobDetails.id = " + id + " returned " + l.size() + " records. " );
		return (FullSDSBatch)l.get(0);
	}
	...
	public String toXMLString(java.util.HashMap domMap)
	{
		...
		String keyClassName = "FullSDSBatch";
	...
	private static void fillSetFromXMLString(java.util.Set set, org.dom4j.Element el, ims.domain.DomainFactory factory, java.util.HashMap domMap) throws Exception
	{
		...
		for(int i=0; i<size; i++) 
		{
			...
			FullSDSBatch domainObject = getFullSDSBatchfromXML(itemEl, factory, domMap);
	...
	public static FullSDSBatch getFullSDSBatchfromXML(String xml, ims.domain.DomainFactory factory, java.util.HashMap domMap) throws Exception
	{
		org.dom4j.Document doc = new org.dom4j.io.SAXReader().read(new org.xml.sax.InputSource(xml));
		return getFullSDSBatchfromXML(doc.getRootElement(), factory, domMap);
	}
	
	public static FullSDSBatch getFullSDSBatchfromXML(org.dom4j.Element el, ims.domain.DomainFactory factory, java.util.HashMap domMap) throws Exception
	{
		if (el == null)
			return null;
		
		String className = el.attributeValue("type");
		if (!FullSDSBatch.class.getName().equals(className))
		{
			Class clz = Class.forName(className);
			if (!FullSDSBatch.class.isAssignableFrom(clz))
				throw new Exception("Element of type = " + className + " cannot be imported using the FullSDSBatch class");
			String shortClassName = className.substring(className.lastIndexOf(".")+1);
			String methodName = "get" + shortClassName + "fromXML";
			java.lang.reflect.Method m = clz.getMethod(methodName, new Class[]{org.dom4j.Element.class, ims.domain.DomainFactory.class, java.util.HashMap.class});
			return (FullSDSBatch)m.invoke(null, new Object[]{el, factory, domMap});
		}

		String impVersion = el.attributeValue("classVersion");
		if(!impVersion.equals(FullSDSBatch.CLASSVERSION))
		{
			throw new Exception("Incompatible class structure found. Cannot import instance.");
		}		
		
		FullSDSBatch ret = null;
		int extId = Integer.parseInt(el.attributeValue("id"));
		String externalSource = el.attributeValue("source");
		ret = (FullSDSBatch)factory.getImportedDomainObject(FullSDSBatch.class, externalSource, extId);	
		if (ret == null)
		{
			ret = new FullSDSBatch();
		}
		String keyClassName = "FullSDSBatch";
		...
		if (impObj != null)
		{
			return (FullSDSBatch)impObj.getDomainObject();
		}
		else
		{
    		impObj = new ims.configuration.ImportedObject();
	...
	public static void fillFieldsfromXML(org.dom4j.Element el, ims.domain.DomainFactory factory, FullSDSBatch obj, java.util.HashMap domMap) throws Exception
	{
