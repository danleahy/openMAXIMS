...
public class HL7TTOIFImpl extends BaseHL7TTOIFImpl
{
	...
	*/
	public void setPatientCareContextTTORecord(ims.clinical.vo.PatientTTOIfVo patientTTOs)
	{
		...
		Set<?> ttaCommentsFromMessage=TTANoteVoAssembler.extractTTANoteSet(factory,patientTTOs.getTTANotes() );
		md.getTTAComments().addAll(ttaCommentsFromMessage);
		
		//http://jira/browse/WDEV-18682
		RTLSummary rtlSummary = getRTLSummary(careContext);
		rtlSummary.setWasJACReceived(Boolean.TRUE);
		
		//WDEV-19370 If at least one TTA has been received, set Received flag to true 
		if(md.getTTAs()!=null)
			md.setTTAReceived(true);
	...
	//http://jira/browse/WDEV-18682
	private RTLSummary getRTLSummary(CareContext careContext)
	{
		DomainFactory factory = getDomainFactory();
		RTLSummary rtlSummary;
		List<?> rtlSummaryList = factory.find("select rtls from RTLSummary rtls left join rtls.careContext cc where cc.id = :CC",
				new String[] {"CC"},new Object[]{careContext.getId()});
		if (rtlSummaryList==null||rtlSummaryList.size()==0||rtlSummaryList.get(0)==null)
		{
			// Create one
			rtlSummary= new RTLSummary();
			rtlSummary.setCareContext(careContext);
		}
		else
		{
			rtlSummary=(RTLSummary)rtlSummaryList.get(0);
		}
		return rtlSummary;
	}
