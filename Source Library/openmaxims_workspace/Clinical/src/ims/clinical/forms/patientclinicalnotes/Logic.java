...
public class Logic extends BaseLogic
{
	...
	protected void onFormOpen(Object[] args) throws ims.framework.exceptions.PresentationLogicException
	{
		...
		//WDEV-19389 
		if (!(form.getGlobalContext().Core.getCurrentCareContextIsNotNull() && form.getGlobalContext().Core.getCurrentCareContext().getEpisodeOfCareIsNotNull() && form.getGlobalContext().Core.getPatientClinicalNotesSearchCriteriaIsNotNull() && form.getGlobalContext().Core.getCurrentCareContext().getEpisodeOfCare().equals(form.getGlobalContext().Core.getPatientClinicalNotesSearchCriteria().getEpisodeOfCare())))
		{
			form.getGlobalContext().Core.setPatientClinicalNotesSearchCriteria(null);
		}
		
		if(form.getGlobalContext().Core.getPatientClinicalNotesSearchCriteriaIsNotNull())
		{
			setSearchCriteria(form.getGlobalContext().Core.getPatientClinicalNotesSearchCriteria());
		}
		//WDEV-19389 - end
		open();
	}	
	...
	protected void onFormDialogClosed(ims.framework.FormName formName, ims.framework.enumerations.DialogResult result) throws ims.framework.exceptions.PresentationLogicException
	{
		...
		//initialise();
		//--------------
		if (!form.getForms().Core.RecordedInErrorDialog.equals(formName)) //WDEV-18834
		{
			form.getLocalContext().setSelectedClinicalNote(null);	//wdev-14296
	...
	protected void onContextMenuItemClick(int menuItemID, ims.framework.Control sender) throws ims.framework.exceptions.PresentationLogicException
	{
		switch (menuItemID)
		{
			...
			//WDEV-18473
			case GenForm.ContextMenus.ClinicalNoting.VIEW_CORRECTION_HISTORY :
				viewCorrectionHistory();
			break;
			//WDEV-18915
			case GenForm.ContextMenus.ClinicalNoting.CORRECT :
				editClinicalNote();
			break;	
			//wdev-20525
			case GenForm.ContextMenus.ClinicalNoting.ADD_SOAP_NOTE:
				 addsoapnote();
			default :
		}
	}
	...
	//WDEV-18473
	private void viewCorrectionHistory() 
	{
		engine.open(form.getForms().Clinical.ViewCorrectionHistoryDialog);
	}
...
*/
	...
	private void updateControlsState()
	{
		...
		form.getContextMenus().getClinicalNotingADD_SOAP_NOTEItem().setVisible(true);		//wdev-20525
		
		if (form.dyngrdNotes().getSelectedRow() != null 
			&& form.dyngrdNotes().getSelectedRow().getValue() != null)
		{
			//WDEV-18473
			Object currentHcpUser = domain.getHcpLiteUser();
			HcpLiteVo hcpLiteVo=null;
			if (currentHcpUser!=null && currentHcpUser instanceof HcpLiteVo)
			{
				hcpLiteVo=(HcpLiteVo) currentHcpUser;
			}
			
			Object currentMosUser = domain.getMosUser();
			MemberOfStaffLiteVo mosLiteVo=null;
			if (currentMosUser!=null && currentMosUser instanceof MemberOfStaffLiteVo)
			{
				mosLiteVo=(MemberOfStaffLiteVo) currentMosUser;
			}
		
			boolean allowEditClinicalNote= form.getGlobalContext().Clinical.getCurrentClinicalNote().getCurrentStatusIsNotNull() && form.getGlobalContext().Clinical.getCurrentClinicalNote().getCurrentStatus().getStatus().equals(ClinicalNotesStatus.DRAFT)
			&& ((hcpLiteVo!=null && hcpLiteVo.equals(form.getGlobalContext().Clinical.getCurrentClinicalNote().getAuthoringInfo().getAuthoringHcp()))
			    || (mosLiteVo!=null && mosLiteVo.equals(form.getGlobalContext().Clinical.getCurrentClinicalNote().getRecordingUser())));
			
			boolean isSameDiscipline = (form.dyngrdNotes().getSelectedRow().getValue() instanceof ClinicalNotesShortVo ) && testDiscipline((ClinicalNotesShortVo) form.dyngrdNotes().getSelectedRow().getValue());
			
			boolean allowCorrectClinicalNote= form.getGlobalContext().Clinical.getCurrentClinicalNote().getCurrentStatusIsNotNull() && (form.getGlobalContext().Clinical.getCurrentClinicalNote().getCurrentStatus().getStatus().equals(ClinicalNotesStatus.ACTIVE) || form.getGlobalContext().Clinical.getCurrentClinicalNote().getCurrentStatus().getStatus().equals(ClinicalNotesStatus.CORRECTED))
			&& ((hcpLiteVo!=null && (hcpLiteVo.equals(form.getGlobalContext().Clinical.getCurrentClinicalNote().getAuthoringInfo().getAuthoringHcp()) || isSameDiscipline) )
			    || (mosLiteVo!=null && mosLiteVo.equals(form.getGlobalContext().Clinical.getCurrentClinicalNote().getRecordingUser()))
				...
				&& domain.getHcpLiteUser() == null)
			{
				...
				{
					form.getContextMenus().getClinicalNotingEDITItem().setVisible(false);
					form.getContextMenus().getClinicalNotingCORRECTItem().setVisible(false);//WDEV-18915
				}
			}
			if (form.getGlobalContext().Clinical.getCurrentClinicalNoteIsNotNull() 
			...
			//WDEV-18473
			form.getContextMenus().getClinicalNotingEDITItem().setVisible(allowEditClinicalNote );//WDEV-18915
			form.getContextMenus().getClinicalNotingCORRECTItem().setVisible(allowCorrectClinicalNote);//WDEV-18915
			form.getContextMenus().getClinicalNotingVIEW_CORRECTION_HISTORYItem().setVisible(Boolean.TRUE.equals(form.getGlobalContext().Clinical.getCurrentClinicalNote().getIsCorrected()));//WDEV-18473
			return;
		}
	...
	private void clearContextMenu()
	{
		...
		form.getContextMenus().getClinicalNotingVIEW_CORRECTION_HISTORYItem().setVisible(false);//WDEV-18473
		form.getContextMenus().getClinicalNotingCORRECTItem().setVisible(false);//WDEV-18915
		form.getContextMenus().getClinicalNotingADD_SOAP_NOTEItem().setVisible(false);	//wdev-20525
	...
	{			
		...
		form.getGlobalContext().Core.setPatientClinicalNotesSearchCriteria(null);//WDEV-19389 
		updateControlsState();	
	}
