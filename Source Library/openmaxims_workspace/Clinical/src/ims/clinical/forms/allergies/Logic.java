...
import ims.clinical.forms.allergies.GenForm.groupFilterEnumeration;
...
import ims.core.vo.AllergiesResultsSearchCriteriaVo;
...
public class Logic extends BaseLogic
{
	...
	protected void onFormOpen(Object[] args) throws ims.framework.exceptions.FormOpenException
	{
		...
		//WDEV-19389 
		if (!(form.getGlobalContext().Core.getPatientShortIsNotNull() && form.getGlobalContext().Core.getAllergiesResultsSearchCriteriaIsNotNull() && form.getGlobalContext().Core.getAllergiesResultsSearchCriteria().getPatientIsNotNull() && form.getGlobalContext().Core.getPatientShort().equals(form.getGlobalContext().Core.getAllergiesResultsSearchCriteria().getPatient())))
			form.getGlobalContext().Core.setAllergiesResultsSearchCriteria(null);
		
		if(form.getGlobalContext().Core.getAllergiesResultsSearchCriteriaIsNotNull())
		{
			setSearchCriteria(form.getGlobalContext().Core.getAllergiesResultsSearchCriteria());
			open();
		}
		else
			open();
	...
	private AllergiesResultsSearchCriteriaVo getSearchCriteria()
	{
		AllergiesResultsSearchCriteriaVo searchCriteria = new AllergiesResultsSearchCriteriaVo();
	...
	private void setSearchCriteria(AllergiesResultsSearchCriteriaVo allergiesResultsSearchCriteria)
	{
		setViewType(allergiesResultsSearchCriteria.getViewType());		
	}
...
//		sb.append("<br>");
	...
	protected void onBtnCancelClick() throws PresentationLogicException
	{
		open();
		initializeHasNoKnownAllergies();
		selectionChanged(form.grdAllergies().getValue() != null ? form.grdAllergies().getValue().getID_PatientAllergy() : null);
	}
	...
	private boolean savePatientCoreClinicalData(boolean bNoKnownAllergies) 
	{
		...
		String[] errors = voPatientNoAllergyInfo.validate(validateUI()); //WDEV-18757 
	 ...
	 * @return
	 */
	private boolean save(PatientAllergy voPatientAllergy, boolean validateAndSavePatientNoAllergies)
	{		
		...
		PatientNoAllergyInfoVo voPatientNoAllergyInfo = null;
		
		if (validateAndSavePatientNoAllergies)
		{
			populatePatientNoAllergyInfo(false);
			voPatientNoAllergyInfo = form.getLocalContext().getPatientNoAllergyInfo();

			errors = voPatientNoAllergyInfo.validate(validateUI()); // wdev-15696 WDEV-18757
...
//			errorList.add("Authoring Date Time is mandatory.");
		...
		{
			errorList.add(form.ctnDetails().customControlAuthoringInfo().getErrors()); 
		}
	...
	private void selectionChanged(Integer patientAllergyId) 
	{
		...
		if(form.getLocalContext().getVoAllergy().getIsCurrentlyActiveAllergy() != null)
		{
			...
			{
				form.btnUpdate().setEnabled(true);	
				form.btnUpdate().setVisible(true); //WDEV-18757 
				form.btnInactivate().setVisible(true);
				form.getContextMenus().getGenericGridUpdateItem().setVisible(true);
			}
			else
	...
	protected void onRadioButtongroupFilterValueChanged() throws PresentationLogicException
	{
		...
		form.getGlobalContext().Core.setAllergiesResultsSearchCriteria(getSearchCriteria());
	}
