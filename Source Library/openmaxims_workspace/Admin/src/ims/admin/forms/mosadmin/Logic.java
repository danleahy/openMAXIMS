...
public class Logic extends BaseLogic
{
	...
	private void clearMenuItems()
	{
		...
		form.getContextMenus().getMosOrganisationSERVICES_AND_TREATMENT_FUNCTIONSItem().setVisible(false); //WDEV-19955
	...
	private void clearContextVariables()
	{
		...
		form.getGlobalContext().setHcp(null);
		form.getLocalContext().setHcp(null);//WDEV-21029
	...
	private void open()
	{
		// set selection back //WDEV-22742
		if (GenForm.GroupTreeFilterEnumeration.rdoMemberOfStaff.equals(form.GroupTreeFilter().getValue()) && form.getGlobalContext().Admin.getSelectedMosIsNotNull())//WDEV-15437
		{
		...
		switch (menuItemID)
		{
			...
			case GenForm.ContextMenus.MosOrganisation.EDIT :
			{
				...
				if (voMos != null)
				{
					...
					form.getContextMenus().getMosOrganisationDEACTIVATEItem().setVisible(false);
					form.getContextMenus().getMosOrganisationSETORGANISATIONSItem().setVisible(false);
					form.getContextMenus().getMosOrganisationSERVICES_AND_TREATMENT_FUNCTIONSItem().setVisible(false);
			...
			case GenForm.ContextMenus.MosOrganisation.SERVICES_AND_TREATMENT_FUNCTIONS :
			{
				if (sender.equals(form.lyrLHS().tabMos().treMemberOfStaff()) && (form.lyrLHS().tabMos().treMemberOfStaff().getSelectedNode() != null))
					voMos = (MemberOfStaffShortVo) form.lyrLHS().tabMos().treMemberOfStaff().getSelectedNode().getValue();

				if (voMos != null && voMos.getHcp() != null)
				{
					engine.open(form.getForms().Admin.HcpServiceFunctionDialog, new Object[]{(HcpRefVo)voMos.getHcp()});
				}
	...
	protected void onImbHCPDlgClick() throws PresentationLogicException
	{
		...
		Hcp voHcp = new Hcp();	
		//WDEV-20296 add: && form.getGlobalContext().getHcp().getHcpType().equals(form.cmbHcpDiciplineDetails().getValue());		
	...
	private void selectionChanged(TreeNode node)
	{
		...
		form.getContextMenus().getMosOrganisationSERVICES_AND_TREATMENT_FUNCTIONSItem().setVisible(voMos != null && voMos.getHcp() != null);
