...
import ims.admin.forms.dischargesummaryschedule.GenForm.grdJobRow;
...
public class Logic extends BaseLogic
{
	...
	protected void onBtnCancelTaskClick() throws PresentationLogicException 
	{
		if (EnvironmentConfig.getScheduledJobs() != null &&
				Boolean.FALSE.equals(EnvironmentConfig.getScheduledJobs())) 
		{
			engine.showMessage("Quartz Scheduler is disabled");
			return;
		}
		...
		try
		{
			...
			sched.deleteJob(jobToAdd.getJobType().toString(), DischargeSummaryScheduleImpl.class.getSimpleName());
			try 
			{
	...
	{				
		...
		try 
		{
			...
			try
			{
				...
				jd = new JobDetail((form.cmbJobType().getValue()).getText(), DischargeSummaryScheduleImpl.class.getSimpleName(), DischargeSummaryScheduleImpl.class);
				jd.setRequestsRecovery(true);		
				jd.setDescription(DischargeSummaryScheduleImpl.class.getSimpleName());
				...
				simpleCron = new SimpleTrigger((form.cmbJobType().getValue()).getText(), DischargeSummaryScheduleImpl.class.getSimpleName(), startingTime, null, SimpleTrigger.REPEAT_INDEFINITELY, Long.parseLong(repeatInterval.toString()));
